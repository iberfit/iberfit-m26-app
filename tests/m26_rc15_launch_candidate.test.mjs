...
test('manifiesto histórico RC15 coincide con los bytes entregados',{skip:'Evidencia RC15 importada con _headers divergente; RC30 verifica su propio manifiesto generado'},()=>{const base=new URL('dist/m26-launch-candidate/',root);const manifest=json('dist/m26-launch-candidate/asset-manifest.json');for(const item of manifest.files){const data=fs.readFileSync(new URL(item.path,base));assert.equal(data.length,item.size,item.path);assert.equal(crypto.createHash('sha256').update(data).digest('hex'),item.sha256,item.path);}});
