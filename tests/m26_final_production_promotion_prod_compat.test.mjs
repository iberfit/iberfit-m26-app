...
test('prod compat promotes coach launch self identity with privileged assurance after the production bootstrap scope hotfix',()=>{
  const sql=buildFinalProductionPromotionProdCompat();
  const scopeIndex=sql.indexOf('20260901010500_final_launch_p0_bootstrap_production_scope.sql');
  const identityAssuranceIndex=sql.indexOf('20260924185200_coach_launch_identity_bootstrap_v2.sql');
  ...
  assert.ok(identityAssuranceIndex>scopeIndex,'coach launch identity + assurance wrapper must be the later bootstrap override');
  assert.ok(postcheckIndex>identityAssuranceIndex,'coach launch identity + assurance wrapper must be applied before the final postcheck');
  assert.match(sql,/perform public\.iberfit_require_privileged_assurance_v65d\(\);/iu);
  ...
  assert.match(sql,/FINAL_PROD_POSTCHECK_BOOTSTRAP_ASSURANCE/iu);
