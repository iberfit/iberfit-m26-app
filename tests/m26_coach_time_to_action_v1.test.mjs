...
test('Hoy Coach abre cliente + destino en un solo gesto cuando la prioridad ya conoce ambos',()=>{
  ...
  assert.doesNotMatch(html,new RegExp(`class="m26-primary-action"[^>]*data-m26-select-client="${clientId}"`,'u'));
});

test('Hoy Coach conserva navegación simple cuando no existe cliente concreto',()=>{
  const html=renderHoyRoute({
    role:'coach',
    appointments:[],
    proposals:[],
