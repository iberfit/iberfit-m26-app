...
for (const [name, delegate] of [
  ...
  test(`${name} scopes privileged assurance to RENOVACION_REGISTRAR`, async () => {
    const sql = await readFile(migrationPath, 'utf8');
    const body = functionBody(sql, name);

    assert.match(body, /v_type\s+text\s*:=\s*upper\(coalesce\(p_command->>'type',\s*''\)\)/i);
    assert.match(
      body,
      /if\s+v_type\s*=\s*'RENOVACION_REGISTRAR'\s+then[\s\S]*?perform\s+public\.iberfit_require_privileged_assurance_v65d\(\);[\s\S]*?end\s+if;/i,
    );
    ...
    assert.equal(assuranceCalls.length, 1, 'wrapper must contain exactly one scoped assurance call');
  });
}
