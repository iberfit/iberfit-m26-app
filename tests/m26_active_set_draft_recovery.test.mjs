...
test('session controller rehydrates set fields after telemetry-driven renders',()=>{
  ...
  assert.match(source,/render=\(\)=>\{baseRender\?\.\(\);hydrateActiveSetDraft\(getContext\(\)\);hydrateFinalFeedbackDraft\(getContext\(\)\);syncLiveAddExerciseControl\(getContext\(\)\);syncFinishControl\(getContext\(\)\);\}/);
