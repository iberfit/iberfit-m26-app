...
test('renewal builder preserves idempotency identity and explicit commercial fields',()=>{
  const command=buildCommercialRenewalCommand({clientId,entityId,baseRevision:2,operationId:'renewal-op-1',renewal:{renewalStatus:'upcoming',renewalDate:'2026-09-20',commercialPlan:'Hybrid 2x',notes:'Review continuity.'}},{role:'coach'});
...
test('renewal command service enforces role and online execution',async()=>{
  const executed=[];
