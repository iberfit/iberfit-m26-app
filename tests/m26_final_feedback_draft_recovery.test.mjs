...
test('controlador captura, persiste e hidrata RPE, comentario, dolor y notas',()=>{
  ...
  assert.ok(source.includes('render=()=>{baseRender?.();hydrateActiveSetDraft(getContext());hydrateFinalFeedbackDraft(getContext());syncLiveAddExerciseControl(getContext());};'));
