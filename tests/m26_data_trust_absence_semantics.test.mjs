...
test('missing state stays explicit for professional roles without changing provenance detail',()=>{
  const html=renderDataTrustStrip({
    source:'wearableDailySummaries',
    providers:['garmin_connect','oura'],
    quality:'sin_datos',
    coverage:0,
    missing:true,
  ...
  assert.match(html,/Limitación:/);
  assert.match(html,/data-data-trust-missing="true"/);
  assert.match(html,/no se convierten en cero/);
});
