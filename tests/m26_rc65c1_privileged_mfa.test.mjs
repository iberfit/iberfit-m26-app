...
test('RC65-C1 FREE decisión privilegiada exige assurance IBERFIT antes del bootstrap privilegiado',()=>{
...
test('RC65-C1 FREE UI sigue integrada, solo dispositivo local y sin QR ni app TOTP',()=>{
  ...
  const challengeWithEmail=renderAccessUi({mode:'mfa-challenge',mfa:{factorId:AUTHENTICATION_FACTOR_ID,privilegedRole:'admin',emailOtpAvailable:true}});
  ...
  assert.match(challengeWithEmail,/data-auth-action="mfa-send-email-code"/u);
  assert.match(challengeWithEmail,/Enviar código al correo asociado/u);
  ...
  for(const markup of [required,challenge,verifiedLoading])assert.doesNotMatch(markup,/Google Authenticator|Authy|qr_code|otpauth/iu);
});
