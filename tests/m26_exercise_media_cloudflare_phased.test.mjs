...
test('fase final prioriza pose, exige zapato suspendido y rechaza toe tap',()=>{const prompt=buildPhasePrompt(exercise,'end',{hasAthleteReference:true,hasPoseReference:true});assert.match(prompt,/Input image 0 is the PRIMARY GEOMETRIC POSE GUIDE/i);assert.match(prompt,/Input image 1 is ONLY the canonical IBERFIT male FACE\/IDENTITY reference/i);assert.match(prompt,/shoe is clearly SUSPENDED ABOVE THE FLOOR/i);assert.match(prompt,/RIGHT moving shoe must be visibly 20–30 cm ABOVE THE FLOOR/i);assert.match(prompt,/continuous dark floor\/background gap under the ENTIRE shoe/i);assert.match(prompt,/25–35 degree hip-abduction angle/i);assert.match(prompt,/NO toe tap/i);assert.match(prompt,/NO both-feet-on-floor pose/i);});

test('guidance final es mayor que inicio y queda dentro del rango seguro',()=>{assert.ok(END_GUIDANCE>START_GUIDANCE);assert.ok(START_GUIDANCE>=1&&END_GUIDANCE<=10);});
