...
test('aplicación pública usa carga absoluta diferida y configuración cerrada',()=>{const entry=text('public/m26/app.js');assert.doesNotMatch(entry,/^import\s+\{createM26Application\}/m);assert.match(entry,/import\('\/src\/m26\/app\/application\.js'\)/);assert.match(entry,/M26_BOOTSTRAP_MODULE_TIMEOUT/);assert.match(text('public/m26/runtime-config.js'),/enabled:\s*false/);assert.match(text('public/m26/runtime-config.example.js'),/qaOnly:\s*true/);});
