...
test('cola persistente conserva operationId y sincroniza al volver online',async()=>{const storage=createMemoryKeyValueStore();const repository=createKeyValueOperationRepository({storage,ownerId:'user-qa'});let calls=0;const bus=createCommandBus({repository,getToken:async()=> 'token',transport:{preflight:async()=>({}),execute:async(_token,command)=>{calls+=1;return {kind:'ack',operationId:command.operationId,remoteRevision:1};}}});const command={type:'EJECUCION_GUARDAR_PROGRESO',entityType:'session_execution',entityId:'e1',clientId:'c1',baseRevision:0,conflictSensitive:false,payload:{progressSnapshot:{id:'e1'}}};const queued=await bus.enqueue(command);assert.equal(queued.queued,true);assert.equal((await bus.pending()).length,1);const result=await bus.flushPending();assert.equal(result.attempted,1);assert.equal(calls,1);assert.equal((await bus.pending()).length,0);});
