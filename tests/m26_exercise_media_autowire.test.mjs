...
import { buildExerciseMediaUpserts } from '../scripts/exercise-media/autowire.mjs';
...
test('autowire enlaza únicamente por exercise_id canónico exacto', () => {
  const catalog = [exercise('squat-a', 'Sentadilla'), exercise('squat-b', 'Sentadilla')];
  const result = buildExerciseMediaUpserts(catalog, manifest([
  ...
  assert.deepEqual(result.upserts.map((item) => item.id), ['squat-a', 'squat-b']);
  assert.equal(result.upserts[0].media.movement.path, 'squat-a/movement.webp');
  assert.equal(result.upserts[1].media.movement.path, 'squat-b/movement.webp');
...
test('autowire rechaza exercise_id desconocido aunque el nombre o slug coincidan', () => {
  ...
  assert.throws(
    () => buildExerciseMediaUpserts(catalog, manifest([{ exercise_id: 'other-squat', slug: 'sentadilla', media: media('other-squat') }])),
...
test('autowire rechaza IDs de media duplicados', () => {
  ...
  assert.throws(
    () => buildExerciseMediaUpserts(catalog, manifest([
...
test('autowire rechaza exercise_id ausente o normalizado implícitamente', () => {
  ...
  assert.throws(
    () => buildExerciseMediaUpserts(catalog, manifest([{ media: media('bench') }])),
    /MEDIA_EXERCISE_ID_INVALID/,
  );
  assert.throws(
    () => buildExerciseMediaUpserts(catalog, manifest([{ exercise_id: ' bench ', media: media('bench') }])),
    /MEDIA_EXERCISE_ID_INVALID/,
  );
});
...
test('autowire exige el contrato visual canónico y QA aprobado', () => {
  const catalog = [exercise('bench')];
  assert.throws(
    () => buildExerciseMediaUpserts(catalog, manifest([{ exercise_id: 'bench', media: media('bench', { qa: { biomechanics: 'pending', visual: 'approved' } }) }])),
    /MEDIA_MANIFEST_INVALID:bench:qa\.biomechanics/,
  );
  assert.throws(
    () => buildExerciseMediaUpserts(catalog, manifest([{ exercise_id: 'bench', media: media('bench', { schema: 'invented.schema' }) }])),
...
test('autowire rechaza rutas que pertenezcan a otro ejercicio', () => {
  ...
  assert.throws(
    () => buildExerciseMediaUpserts(catalog, manifest([{ exercise_id: 'rdl', media: wrong }])),
...
test('autowire es determinista, no muta el catálogo y reporta faltantes', () => {
  ...
  const result = buildExerciseMediaUpserts(catalog, manifest([{ exercise_id: 'rdl', media: media('rdl') }]), { generatedAt: 'fixed' });

  assert.deepEqual(catalog, before);
  assert.equal(result.schema, 'iberfit.exercise.media.autowire.v2');
  assert.deepEqual(result.totals, { catalog: 2, matched: 1, missing: 1 });
  assert.deepEqual(result.upserts.map((item) => item.id), ['rdl']);
...
test('autowire rechaza manifest sin versión de contrato v2', () => {
  ...
  assert.throws(
    () => buildExerciseMediaUpserts(catalog, { schema: 'iberfit.exercise.media.manifest.v1', items: [] }),
