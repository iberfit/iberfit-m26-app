...
test('real Admin QA gate is pinned to QA and canonical Canary origin',()=>{
...
test('real Admin QA gate only permits read-only RPCs plus WebAuthn',()=>{
  ...
  assert.match(spec,/iberfit_communication_bootstrap_v14/u);
...
test('real Admin QA gate uses current source under secure origin and blocks service role',()=>{
  assert.match(spec,/BUILD_ROOT=path\.resolve\('\.tmp\/rc64-current-surface'\)/u);
  assert.match(spec,/url\.origin===CANARY_ORIGIN/u);
  assert.match(spec,/fulfillCurrentSource\(route,url\)/u);
...
test('real Admin QA gate never submits business forms',()=>{
...
test('real Admin QA gate requires explicit Coach Admin app choice before entering Admin',()=>{
  ...
  assert.match(spec,/Client app must remain isolated and must not be offered/u);
  assert.match(spec,/Primary Coach app remains inert until an app is chosen/u);
  assert.match(spec,/evidence\.appChoice=\{shown:true,authorized:\['coach','admin'\],selected:'admin'\}/u);
});

test('real Admin QA gate closes the non-modal guide before testing Settings hit targets',()=>{
  assert.match(spec,/\[data-m26-guided-tour\]/u);
  assert.match(spec,/aria-modal','false'/u);
  assert.match(spec,/\[data-m26-guided-tour-close\]/u);
  assert.match(spec,/guidedTourClosedBeforeSettings=true/u);
});
