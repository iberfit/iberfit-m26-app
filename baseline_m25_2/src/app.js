...
function iriControl(section, field, label, type = 'text', config = {}) {
  const interpretation = state.iriAssessment.sections.interpretacion || {};
  const scoring = section === 'interpretacion' ? calculateIriScore(state.iriAssessment) : null;
  const automaticInterpretation = section === 'interpretacion' && !interpretation.overrideManual;
  const derivedValue = section === 'interpretacion' && field === 'score'
    ? scoring?.score ?? ''
    : section === 'interpretacion' && field === 'calidadDatos'
      ? scoring?.dataQuality ?? ''
      : section === 'interpretacion' && field === 'clasificacion' && automaticInterpretation
        ? scoring?.automaticClassification ?? ''
        : state.iriAssessment.sections[section]?.[field] ?? '';
  const value = derivedValue;
  const locked = ['aprobado', 'publicado', 'retirado'].includes(state.iriAssessment.status);
  const derivedLocked = section === 'interpretacion' && (
    field === 'score'
    || field === 'calidadDatos'
    || (field === 'clasificacion' && automaticInterpretation)
    || (field === 'motivoOverride' && !interpretation.overrideManual)
  );
  const readonly = config.readonly || locked || derivedLocked;
...
async function handleAction(action) {
  ...
  try {
    ...
    if (action === 'save-iri-draft') {
      ...
      let iriAssessment = synchronizeIriDerivedFields(before);
