...
export function synchronizeIriDerivedFields(assessment) {
  const scoring = calculateIriScore(assessment);
  const section = assessment.sections.interpretacion;
  const manual = Boolean(section.overrideManual);
  const nextSection = {
    ...section,
    clasificacion: manual && text(section.clasificacion)
      ? section.clasificacion
      : scoring.automaticClassification,
    motivoOverride: manual ? section.motivoOverride : '',
    score: scoring.score ?? '',
    calidadDatos: scoring.dataQuality,
  };
  return {
    ...assessment,
    updatedAt: new Date().toISOString(),
...
export function applyDerivedIriInterpretation(assessment) {
  const synchronized = synchronizeIriDerivedFields(assessment);
  const derived = deriveIriInterpretation(synchronized);
  const section = synchronized.sections.interpretacion;
  const nextSection = {
    ...section,
  ...
  return {
    ...
    revision: Number(assessment.revision || 0) + 1,
    updatedAt: new Date().toISOString(),
    sections: { ...synchronized.sections, interpretacion: nextSection },
  };
}
