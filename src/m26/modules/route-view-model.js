...
import {buildIri2DecisionLog,buildIri2PlanningSeed} from '../workflows/iri-2-longitudinal.js';
...
// RC71_0_ROUTE_VM_CASES_BEGIN
  ...
  if (area === 'planificacion') {
    ...
    const confirmedDecisionDrafts=assessments
      ...
      .map((record)=>confirmedFirstSessionDraft(record,clientId));
    const decisionLog=buildIri2DecisionLog({assessments:confirmedDecisionDrafts});
    const iriPlanningSeed=canEdit?buildIri2PlanningSeed({decisionLog,profile}):null;
    return Object.freeze({
      kind: 'planificacion',
      clientId,
      role,
