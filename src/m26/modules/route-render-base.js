...
export function renderActivityRoute(vm){
  ...
  return `<div class="m26-route m30-activity-route" data-activity-role="${escapeHtml(vm.role||'unknown')}">
    ...
    <section class="m26-content-grid">
      <form class="m26-panel m30-activity-primary" data-engagement-form="checkin" aria-labelledby="m30-activity-checkin-title"><div class="m26-panel-heading"><div><p class="m26-eyebrow">Registro de bienestar</p><h2 id="m30-activity-checkin-title">Cómo estás hoy</h2></div></div><div class="m26-field-grid m30-wellbeing-fields">
...
export function renderIriRoute(vm) {
  ...
  const editor=vm.canEdit?`<form class="m26-panel m26-iri-wizard m30-iri-wizard" data-workflow-form="iri" data-iri-step-index="0" aria-label="Evaluación IRI guiada"><input type="hidden" name="entityId" value="${escapeHtml(current.id||'')}"><div class="m26-stepper m30-iri-stepper" role="list" aria-label="Etapas de la evaluación IRI">${['Expediente','Entrevista','Composición','Movilidad','Fuerza','Cardio','Revisión'].map((labelText,index)=>`<button type="button" role="listitem" data-iri-step-jump="${index}"${index===0?' class="is-active"':''}><span>${index+1}</span><small>${labelText}</small></button>`).join('')}</div><div class="m26-progress-track m30-iri-progress" aria-label="Progreso de la evaluación IRI"><i data-iri-progress data-progress-step="1"></i></div><div class="m30-iri-active-step-shell">${steps.join('')}</div><div class="m26-wizard-actions m30-iri-actions" aria-label="Acciones de la evaluación IRI"><button type="button" data-workflow-action="iri-prev" disabled aria-disabled="true">Anterior</button><button type="button" data-workflow-action="save-iri-draft">Guardar borrador</button><button type="button" class="m26-primary-action" data-workflow-action="iri-next">Validar y continuar</button><button type="submit" class="m26-primary-action" data-workflow-action="complete-iri" hidden${current.id?'':' disabled aria-disabled="true"'}>Confirmar evaluación IRI</button></div>${current.id?'':'<p class="m26-notice is-warning">El expediente debe devolver una entidad IRI remota antes de poder confirmar. El borrador y los informes internos pueden prepararse igualmente.</p>'}${workflowStatus('iri')}</form>`:'';
...
export function renderLibraryRoute(vm){
  ...
  return `<div class="m26-route m30-library-route" data-library-role="${escapeHtml(vm.role||'coach')}"><section class="m26-route-intro m30-library-intro"><div><p class="m26-eyebrow">Biblioteca visual</p><h2>Ejercicios IBERFIT</h2><p>Organizados por musculatura principal, con indicaciones y referencias visuales validadas. El catálogo sigue siendo la fuente canónica y no admite escritura libre en las sesiones.</p></div>${badge(`${vm.total} ejercicios`,'neutral')}</section><section class="m26-panel m30-library-panel"><div class="m26-library-controls m30-library-controls" aria-label="Buscar y filtrar ejercicios"><label class="m30-library-search">Buscar ejercicio<input type="search" data-library-search autocomplete="off" spellcheck="false" enterkeyhint="search" autocapitalize="none" aria-describedby="m26-library-status"></label><label>Material<select data-library-filter="equipment"><option value="">Todo</option><option value="sin material">Sin material</option><option value="trx">TRX</option><option value="mancuerna">Mancuernas</option><option value="banda">Bandas</option><option value="máquina">Máquina</option></select></label><label>Patrón<select data-library-filter="pattern"><option value="">Todos</option><option value="sentadilla">Sentadilla</option><option value="empuje">Empuje</option><option value="tracción">Tracción</option><option value="bisagra">Bisagra</option><option value="core">Core</option></select></label><label>Referencia visual<select data-library-filter="visual"><option value="">Todas</option><option value="with-image">Con imagen validada</option><option value="without-image">Sin imagen</option></select></label><button type="button" class="m30-library-clear" data-library-clear>Limpiar filtros</button></div><p id="m26-library-status" class="m30-library-status" data-library-status role="status" aria-live="polite">Mostrando los ${vm.total} ejercicios del catálogo, agrupados por musculatura principal. Escribe para filtrar.</p><div class="m26-library-groups m30-library-groups" data-library-grid>${groups||emptyState('Biblioteca no cargada','No se pudo leer el catálogo local.')}</div>${credit}</section></div>`;
}
