...
export function clientProgressPresentationStage(summary,{timelineLength=0,exerciseCount=0}={}){
  if(!summary){
    return Object.freeze({
      stage:'starting',
...
function iriDecisionChangeCopy(entry={}){
  ...
  if(changes.priorityDetailsChanged)parts.push('Detalle de prioridades actualizado');
...
function iriPriorityDomainLabel(value){
  return ({general:'General',composition:'Composición corporal',mobility:'Movimiento y movilidad',strength:'Fuerza',cardio:'Cardio',recovery:'Recuperación / contexto',adherence:'Adherencia',other:'Otro'})[String(value||'').toLowerCase()]||'General';
}
function iriPriorityStatusLabel(value){
  return ({active:'Activa',maintain:'Mantener',completed:'Completada',paused:'En pausa'})[String(value||'').toLowerCase()]||'Activa';
}
function iriDecisionPriorities(entry={}){
  const records=Array.isArray(entry.priorityRecords)?entry.priorityRecords.filter((item)=>item?.label).slice(0,4):[];
  if(records.length)return `<div class="m26-stack">${records.map((item)=>`<article class="m26-panel m26-panel-soft"><div class="m26-panel-heading"><div><p class="m26-eyebrow">${escapeHtml(iriPriorityDomainLabel(item.domain))}</p><h4>${escapeHtml(item.label)}</h4></div>${badge(iriPriorityStatusLabel(item.status),item.status==='completed'?'success':item.status==='paused'?'warning':'neutral')}</div>${item.rationale?`<p><strong>Motivo:</strong> ${escapeHtml(decisionExcerpt(item.rationale,300))}</p>`:''}${item.target?`<p><strong>Objetivo:</strong> ${escapeHtml(decisionExcerpt(item.target,260))}</p>`:''}${item.strategy?`<p><strong>Estrategia:</strong> ${escapeHtml(decisionExcerpt(item.strategy,360))}</p>`:''}${item.reviewDate?`<p><strong>Revisión:</strong> ${escapeHtml(item.reviewDate)}</p>`:''}</article>`).join('')}</div>`;
  return Array.isArray(entry.priorities)&&entry.priorities.length
...
function iriStructuredPriorityEditor(){
  const domains=[['general','General'],['composition','Composición corporal'],['mobility','Movimiento y movilidad'],['strength','Fuerza'],['cardio','Cardio'],['recovery','Recuperación / contexto'],['adherence','Adherencia'],['other','Otro']];
  const statuses=[['active','Activa'],['maintain','Mantener'],['completed','Completada'],['paused','En pausa']];
  return `<section class="m26-wide m26-panel m26-panel-soft" data-iri-structured-priorities><div class="m26-panel-heading"><div><p class="m26-eyebrow">IRI 2.0</p><h4>Prioridades estructuradas</h4><p>Enriquece hasta las cuatro primeras prioridades escritas arriba. Los campos vacíos permanecen vacíos: IBERFIT no inventa motivos, objetivos ni estrategias.</p></div></div><div class="m26-test-cards">${[1,2,3,4].map((slot)=>`<section><h4>Prioridad ${slot}</h4><div class="m26-field-grid">${iriSelect('Dominio',`priority${slot}Domain`,domains)}${iriSelect('Estado',`priority${slot}Status`,statuses)}${iriTextarea('Motivo',`priority${slot}Rationale`,{max:600})}${iriTextarea('Objetivo medible',`priority${slot}Target`,{max:400})}${iriTextarea('Estrategia',`priority${slot}Strategy`,{max:800})}${iriInput('Fecha de revisión',`priority${slot}ReviewDate`,{type:'date'})}</div></section>`).join('')}</div></section>`;
}
...
function iriPlanningContextPanel(seed){
  if(!seed)return '';
  const priorities=Array.isArray(seed.priorities)&&seed.priorities.length
    ?`<ul>${seed.priorities.slice(0,4).map((item)=>`<li>${escapeHtml(item)}</li>`).join('')}</ul>`
    :'<p>Sin prioridades registradas.</p>';
  const frequency=seed.suggestedWeeklyFrequency
    ?`${seed.suggestedWeeklyFrequency} sesiones/semana`
    :'Por revisar';
  const duration=seed.suggestedSessionDurationMinutes
    ?`${seed.suggestedSessionDurationMinutes} min`
    :'Por revisar';
  return `<section class="m26-panel m26-panel-soft" data-iri-planning-context>
    <div class="m26-panel-heading">
      <div>
        <p class="m26-eyebrow">Contexto IRI confirmado</p>
        <h2>Base para este ciclo</h2>
        <p><strong>Evaluación confirmada:</strong> ${escapeHtml(seed.sourceAssessmentDate||'Sin fecha')}</p>
        <p>IBERFIT propone estos datos como punto de partida; no crea, valida ni publica un plan automáticamente.</p>
      </div>
      ${badge('Revisión del Coach obligatoria','warning')}
    </div>
    <div class="m26-content-grid">
      <article class="m26-panel"><h3>Prioridades confirmadas</h3>${priorities}</article>
      <article class="m26-panel"><h3>Decisión de entrenamiento</h3><p>${escapeHtml(seed.trainingImplications||'Sin implicación específica registrada.')}</p><p><strong>Frecuencia orientativa:</strong> ${escapeHtml(frequency)} · <strong>Duración del expediente:</strong> ${escapeHtml(duration)}</p><p><strong>Próxima reevaluación:</strong> ${escapeHtml(seed.reevaluationDate||'Por definir')}</p></article>
    </div>
  </section>`;
}
...
export function renderPlanningRoute(vm){
  ...
  return `<div class="m26-route m26-planning-workbench-v2" data-planning-workbench-v2>
    ...
    ${editor}
    ${!isClient?'<p class="m26-planning-publication-note">Aprobar no lo hace visible para el cliente.</p>':''}
...
export function renderLibraryRoute(vm){
  ...
  const canCreate=['coach','admin'].includes(String(vm.role||''));
  const creation=canCreate?`<section class="m26-panel m26-panel-soft" data-exercise-create-panel><div class="m26-panel-heading"><div><p class="m26-eyebrow">Catálogo IBERFIT</p><h2>Crear ejercicio personalizado</h2><p>Añádelo una sola vez al catálogo canónico. Quedará disponible inmediatamente para planificar sesiones y pendiente de revisión profesional.</p></div>${badge('Coach · Admin','neutral')}</div><form data-exercise-create-form><div class="m26-field-grid"><label>Nombre del ejercicio<input name="nameEs" required minlength="2" maxlength="160" autocomplete="off" spellcheck="false"></label><label>Patrón<select name="pattern" required><option value="">Selecciona</option><option value="sentadilla">Sentadilla</option><option value="bisagra">Bisagra</option><option value="empuje horizontal">Empuje horizontal</option><option value="empuje vertical">Empuje vertical</option><option value="tracción horizontal">Tracción horizontal</option><option value="tracción vertical">Tracción vertical</option><option value="core">Core</option><option value="movilidad">Movilidad</option><option value="potencia">Potencia</option><option value="complementario">Complementario</option></select></label><label>Objetivo<select name="intent" required><option value="">Selecciona</option><option value="fuerza">Fuerza</option><option value="hipertrofia">Hipertrofia</option><option value="técnica">Técnica</option><option value="movilidad">Movilidad</option><option value="potencia">Potencia</option><option value="acondicionamiento">Acondicionamiento</option></select></label><label>Material<input name="equipment" required minlength="2" maxlength="80" placeholder="Ej. mancuerna, TRX, sin material"></label><label>Dificultad<select name="difficulty" required><option value="">Selecciona</option><option value="inicial">Inicial</option><option value="media">Media</option><option value="avanzada">Avanzada</option></select></label><label>Musculatura principal<input name="primaryMuscles" maxlength="960" placeholder="Ej. glúteos, cuádriceps"></label></div><details><summary>Completar indicaciones técnicas</summary><div class="m26-field-grid"><label>Musculatura secundaria<input name="secondaryMuscles" maxlength="960" placeholder="Separada por comas"></label><label>Claves técnicas<textarea name="cues" maxlength="2880" rows="3" placeholder="Una por línea o separadas por comas"></textarea></label><label>Ejecución<textarea name="instructionsEs" maxlength="4800" rows="4" placeholder="Pasos de ejecución"></textarea></label><label>Precauciones<textarea name="precautions" maxlength="2880" rows="3" placeholder="Solo cuando corresponda"></textarea></label></div></details><div class="m26-notice"><strong>Catálogo, no texto libre</strong><p>El ejercicio se crea con un ID estable IBERFIT, sin imagen hasta su validación. Las sesiones siguen aceptando únicamente ejercicios presentes en el catálogo.</p></div><button type="submit" class="m26-primary-action">Crear y añadir al catálogo</button><p data-exercise-create-status role="status" aria-live="polite"></p></form></section>`:'';
  return `<div class="m26-route"><section class="m26-route-intro"><div><p class="m26-eyebrow">Biblioteca visual</p><h2>Ejercicios IBERFIT</h2><p>Organizados por musculatura principal, con indicaciones y referencias visuales validadas. El catálogo sigue siendo la fuente canónica y no admite escritura libre en las sesiones.</p></div>${badge(`${vm.total} ejercicios`,'neutral')}</section>${creation}<section class="m26-panel"><div class="m26-library-controls"><label>Buscar ejercicio<input type="search" data-library-search autocomplete="off" spellcheck="false" aria-describedby="m26-library-status"></label><label>Material<select data-library-filter="equipment"><option value="">Todo</option><option value="sin material">Sin material</option><option value="trx">TRX</option><option value="mancuerna">Mancuernas</option><option value="banda">Bandas</option><option value="máquina">Máquina</option></select></label><label>Patrón<select data-library-filter="pattern"><option value="">Todos</option><option value="sentadilla">Sentadilla</option><option value="empuje">Empuje</option><option value="tracción">Tracción</option><option value="bisagra">Bisagra</option><option value="core">Core</option></select></label><label>Referencia visual<select data-library-filter="visual"><option value="">Todas</option><option value="with-image">Con imagen validada</option><option value="without-image">Sin imagen</option></select></label><button type="button" data-library-clear>Limpiar filtros</button></div><div class="m26-library-groups" data-library-grid>${groups||emptyState('Biblioteca no cargada','No se pudo leer el catálogo local.')}</div><p id="m26-library-status" data-library-status role="status" aria-live="polite">Mostrando los ${vm.total} ejercicios del catálogo, agrupados por musculatura principal. Escribe para filtrar.</p>${credit}</section></div>`;
}
