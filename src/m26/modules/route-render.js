...
function wearableDailyRecordCard(item){const metrics=item?.metrics||{};const provider=castilianSourceLabel(item?.provider||'normalized_file');return `<article class="m26-wearable-day" data-wearable-date="${escapeHtml(item?.date||'')}"><header><div><p class="m26-eyebrow">${escapeHtml(provider)}</p><h3>${escapeHtml(safeDateLabel(item?.date))}</h3></div>${badge(escapeHtml(item?.quality||'limitada'),item?.quality==='alta'?'success':'neutral')}</header><div class="m26-field-grid">${wearableMetric('Pasos',metrics.steps)}${wearableMetric('Minutos activos',metrics.activeMinutes,' min')}${wearableMetric('Sueño',metrics.sleepMinutes,' min')}${wearableMetric('FC en reposo',metrics.restingHeartRate,' lpm')}${wearableMetric('VFC',metrics.hrvMs,' ms')}${wearableMetric('Energía activa',metrics.activeEnergyKcal,' kcal')}${wearableMetric('Entrenamiento',metrics.workoutMinutes,' min')}</div></article>`;}
...
export function renderActivityRoute(vm){
  ...
  const dailyRecords=wearable.dailyRecords?.length?`<section class="m26-panel m26-wearable-history"><div class="m26-panel-heading"><div><p class="m26-eyebrow">Expediente confirmado</p><h2>Últimos registros diarios</h2><p>Valores individuales normalizados; no son estimaciones reconstruidas desde promedios.</p></div>${badge(`${wearable.dailyRecords.length} registro${wearable.dailyRecords.length===1?'':'s'}`,'success')}</div><div class="m26-wearable-day-grid">${wearable.dailyRecords.map(wearableDailyRecordCard).join('')}</div></section>`:'';
