...
function clientOnboardingForm() {
  return `<details class="m26-panel m26-onboarding" data-client-onboarding>
    <summary><span><small>Alta segura</small><strong>Nueva clienta o nuevo cliente</strong></span><span class="m26-summary-action">Crear expediente</span></summary>
...
export function renderClientsRoute(vm) {
  const content = vm.clients.length
    ...
    : emptyState('Todavía no hay clientes visibles', 'Crea el primer expediente para comenzar la evaluación inicial.');
  return `<div class="m26-route"><section class="m26-route-intro"><div><p class="m26-eyebrow">Cartera autorizada</p><h2>Clientes y seguimiento</h2><p>Expedientes confirmados dentro de tus permisos.</p></div>${badge(`${vm.clients.length} cliente${vm.clients.length === 1 ? '' : 's'}`, 'neutral')}</section>${vm.canCreate?clientOnboardingForm():''}<section class="m26-panel"><label>Buscar cliente<input type="search" data-client-search autocomplete="off" spellcheck="false" aria-describedby="m26-client-search-status" placeholder="Nombre, modalidad o estado"></label><p id="m26-client-search-status" data-client-search-status role="status" aria-live="polite">Mostrando ${countLabel(vm.clients.length,'cliente','clientes')}.</p>${content}</section></div>`;
}
...
function iriInput(labelText,name,{type='text',value='',min=null,max=null,step=null,required=false,wide=false,help='',readonly=false}={}){
  const attributes=[`name="${escapeHtml(name)}"`,`type="${escapeHtml(type)}"`,min!==null?`min="${escapeHtml(min)}"`:'',max!==null?`max="${escapeHtml(max)}"`:'',step!==null?`step="${escapeHtml(step)}"`:'',required?'required':'',readonly?'readonly':''].filter(Boolean).join(' ');
  return `<label${wide?' class="m26-wide"':''}>${escapeHtml(labelText)}<input ${attributes} value="${escapeHtml(value)}">${help?`<small>${escapeHtml(help)}</small>`:''}</label>`;
}
function iriTextarea(labelText,name,{wide=true,max=1200,placeholder=''}={}){return `<label${wide?' class="m26-wide"':''}>${escapeHtml(labelText)}<textarea name="${escapeHtml(name)}" maxlength="${max}" placeholder="${escapeHtml(placeholder)}"></textarea></label>`;}
function iriSelect(labelText,name,options,{value='',wide=false,required=false}={}){return `<label${wide?' class="m26-wide"':''}>${escapeHtml(labelText)}<select name="${escapeHtml(name)}"${required?' required':''}>${options.map(([key,labelText])=>`<option value="${escapeHtml(key)}"${key===value?' selected':''}>${escapeHtml(labelText)}</option>`).join('')}</select></label>`;}
...
export function renderIriRoute(vm) {
  const current=vm.current||{};const summary=vm.currentSummary;const profile=vm.profile||{};const sexValue=current.sexForNorms||current.sex_for_norms||profile.sexForNorms||'';
