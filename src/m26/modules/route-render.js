  ...
  const nextAction = next
    ...
    : proposalCount
      ...
      : client && clientIriState(client) !== 'completed'
        ...
        : { area: isClient ? 'sesion' : 'clientes', title: isClient ? 'Revisar tus sesiones' : 'Revisar cartera de clientes', copy: isClient ? 'Consulta las sesiones que ya están publicadas.' : 'Abre un expediente para decidir el siguiente paso.', label: isClient ? 'Ver sesiones' : 'Ver clientes' };
  const stats = [
    stat('Sesiones confirmadas hoy', vm.appointments.length, vm.appointments.length ? 'Listas para realizar' : 'No hay sesiones hoy'),
    next ? stat('Próxima cita confirmada', next.dateLabel, next.title) : stat('Próxima cita confirmada', 'Sin cita programada', 'Programa o confirma la siguiente cita'),
  ];
  if (isClient) stats.push(stat('Tu modalidad', client?.modality || 'Sin modalidad', 'Según tu expediente'));
...
function clientOnboardingForm() {
  return `<details class="m26-panel m26-onboarding" data-client-onboarding>
    ...
    <form data-workflow-form="client-onboarding" class="m26-onboarding-form">
...
function iriTextarea(labelText,name,{wide=true,max=1200,placeholder='',required=false}={}){return `<label${wide?' class="m26-wide"':''}>${requiredLabel(labelText,required)}<textarea name="${escapeHtml(name)}" maxlength="${max}" placeholder="${escapeHtml(placeholder)}"${required?' required aria-required="true"':''}></textarea></label>`;}
...
function protocolGuide(title,steps,meta=''){return `<details class="m26-protocol-guide"><summary>${escapeHtml(title)} · ver protocolo</summary><ol>${steps.map((step)=>`<li>${escapeHtml(step)}</li>`).join('')}</ol>${meta?`<p>${escapeHtml(meta)}</p>`:''}</details>`;}

export function renderIriRoute(vm) {
  const current=vm.current||{};const summary=vm.currentSummary;const profile=vm.profile||{};const sexValue=current.sexForNorms||current.sex_for_norms||profile.sexForNorms||'';const currentBody=current?.body&&typeof current.body==='object'?current.body:current;const iriConfirmed=Boolean(currentBody?.firstSessionCompletedAt||currentBody?.first_session_completed_at)&&summary?.coverageCount===3;
  ...
  return `<div class="m26-route"><section class="m26-route-intro"><div><p class="m26-eyebrow">Primera sesión y diagnóstico</p><h2>Índice de Rendimiento IBERFIT</h2><p>Flujo guiado, reproducible y trazable desde el expediente hasta los dos informes automáticos.</p></div>${badge(summary?summary.coverageLabel:'Pendiente',summary?.coverageCount===3?'success':'warning')}</section>${personContext}<section class="m26-content-grid"><article class="m26-panel m26-panel-soft"><p class="m26-eyebrow">Última evaluación confirmada</p><h2>${escapeHtml(summary?.dateLabel||'Sin fecha')}</h2><p>${escapeHtml(summary?.coverageLabel||'No hay evaluación confirmada.')}</p>${domainCards}<p class="m26-notice">No se presenta una puntuación global automática cuando la evidencia no cubre dominios comparables.</p></article><section class="m26-panel"><div class="m26-panel-heading"><div><p class="m26-eyebrow">Historial</p><h2>Evaluaciones registradas</h2></div>${badge(countLabel(vm.history.length,'registro','registros'),'neutral')}</div>${recordList(vm.history,'Sin evaluaciones IRI')}</section></section>${editor}</div>`;
}

export function renderPlanningRoute(vm){
  const isClient=vm.role==='client';
  const editor=vm.canEdit?`<form class="m26-panel m26-panel-soft" data-workflow-form="planning"><div class="m26-panel-heading"><div><p class="m26-eyebrow">Entrenador</p><h2>Preparar ciclo de entrenamiento</h2><p>Validar crea una versión interna. Después deberá aprobarse y publicarse de forma expresa.</p></div></div><input type="hidden" name="entityId" value="${escapeHtml(vm.currentCycle?.id||'')}"><div class="m26-field-grid"><label>Nombre del ciclo<input name="name" maxlength="120" value="${escapeHtml(vm.currentCycle?.name||'')}" required></label><label>Inicio<input type="date" name="startDate" required></label><label>Fin<input type="date" name="endDate" required></label><label>Modalidad<select name="modality" required><option value="presencial">Presencial</option><option value="hibrido" selected>Híbrido</option><option value="online">Online</option></select></label><label>Frecuencia semanal<input type="number" name="weeklyFrequency" min="1" max="14" value="2" required></label><label>Duración habitual (min)<input type="number" name="sessionDurationMinutes" min="20" max="240" value="60" required></label><label class="m26-wide">Objetivo<textarea name="goal" maxlength="500" required>${escapeHtml(vm.currentCycle?.goal||'')}</textarea></label></div><button type="submit" class="m26-primary-action" data-workflow-action="validate-plan">Validar borrador</button>${workflowStatus('planning')}</form>`:'';
