...
import {createProgressiveOnboardingController} from '../onboarding/progressive-onboarding.js';
...
function qaStage(stage){
  ...
  try{
    ...
    if(typeof hook==='function')void hook(value);
  }catch{}
}
...
export const AUTH_BUSY_WATCHDOG_MS=18_000;

export function createAuthBusyWatchdog({
  timeoutMs=AUTH_BUSY_WATCHDOG_MS,
  setTimeoutFn=globalThis.setTimeout,
  clearTimeoutFn=globalThis.clearTimeout,
  onTimeout=()=>{},
}={}){
  const safeTimeout=Math.max(5_000,Math.min(Number(timeoutMs)||AUTH_BUSY_WATCHDOG_MS,30_000));
  let timer=null;
  let activeId=0;
  function clear(){
    if(timer!==null){
  ...
  function isCurrent(id){return id===activeId;}
  return Object.freeze({begin,complete,invalidate,isCurrent,clear,timeoutMs:safeTimeout});
}
...
export async function createM26Application({root=document.querySelector('#app'),runtimeConfig=globalThis.__IBERFIT_M26_RUNTIME__||{},locationLike=globalThis.location,historyLike=globalThis.history}={}){
  ...
  function yieldInteractionTurn({timeoutMs=96}={}){
    return new Promise((resolve)=>{
      const windowLike=root.ownerDocument?.defaultView||globalThis.window||globalThis;
      let settled=false;
      let timer=null;
      ...
      timer=globalThis.setTimeout?.(finish,Math.max(32,Math.min(Number(timeoutMs)||96,250)))??null;
      if(typeof windowLike?.requestAnimationFrame==='function'){
        windowLike.requestAnimationFrame(finish);
        return;
      }
  ...
  async function mountAuthenticatedControllersProgressively(entries,{authAttemptId=null}={}){
    for(const [name,controller] of entries){
      await yieldInteractionTurn();
      if(authAttemptId!==null&&!authWatchdog.isCurrent(authAttemptId))throw new Error('M26_AUTH_ATTEMPT_SUPERSEDED');
      controller?.mount?.();
      qaStage(`rc64-controller-${String(name||'unknown').replace(/[^a-z0-9-]+/giu,'-')}-ready`);
    }
  }
  ...
  function renderRoute(shellVm,state){
    ...
    const markup=renderRouteView(routeVm);
    qaStage('rc64-route-html-ready');
    return markup;
  }
  ...
  async function setupAuthenticated(){
    ...
    if(root?.dataset)delete root.dataset.m26InteractionReady;
    ...
    await yieldInteractionTurn();
    if(root?.dataset)root.dataset.m26InteractionReady='true';
    root.dispatchEvent(new CustomEvent('m26:interaction-ready',{bubbles:false}));
    qaStage('rc64-shell-interaction-ready');
    ...
    await mountAuthenticatedControllersProgressively([
      ['workflow',workflow],
      ...
      ['iri-reports',iriExternalReports],
      ['rc39',rc39],
