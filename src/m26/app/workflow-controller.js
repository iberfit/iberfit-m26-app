...
export function createWorkflowController({
  ...
  function syncIriConditionalFields(form){
    if(!form)return;
    const bodySkipped=syncIriSkippedGroup(form,{toggleName:'bodyCompositionSkipped',fieldNames:['weightKg','heightCm','bodyFatPercent','leanMassKg','muscleMassKg','bodyWaterPercent','waistCm','visceralFatLevel','bodyCompositionMethod','bodyCompositionDevice','measurementConditions','bodyCompositionAttachment','bodyCompositionNotes'],reasonName:'bodyCompositionSkipReason'});
    const mobilitySkipped=syncIriSkippedGroup(form,{toggleName:'mobilitySkipped',fieldNames:['ankleLeft1','ankleLeft2','ankleLeft3','ankleRight1','ankleRight2','ankleRight3','posteriorLeft1','posteriorLeft2','posteriorLeft3','posteriorRight1','posteriorRight2','posteriorRight3','anklePain','ankleCompensation','posteriorPain','thomasLeft','thomasRight','thomasPelvicControl','thomasPain','hipRotationResult','hipRotationPain','hipRotationCompensation','squatDepth','squatHeels','squatKnees','squatTrunk','squatShift','squatAssistanceResponse','squatPain','mobilityNotes'],reasonName:'mobilitySkipReason'});
    const strengthSkipped=syncIriSkippedGroup(form,{toggleName:'strengthSkipped',fieldNames:['chairStand30s','chairHeightCm','chairStandValid','chairStandNotes','pushVariant','pushUps','pushSupportHeightCm','pushValid','pushNotes','trxRowRepetitions','trxHandleHeightCm','trxHeelDistanceCm','trxPosition','trxValid','trxNotes','frontPlankSeconds','sidePlankLeftSeconds','sidePlankRightSeconds','coreQuality','corePain','posteriorChainProtocol','posteriorChainSeconds','posteriorEquipmentCompatible','posteriorNotPerformedReason','posteriorChainPain','strengthNotes'],reasonName:'strengthSkipReason'});
    const cardioSkipped=syncIriSkippedGroup(form,{toggleName:'cardioSkipped',fieldNames:['cardioProtocol','stepHeightCm','cadenceBpm','cardioDurationSeconds','restingHr','stepFinalHr','stepOneMinuteHr','twoMinuteHr','cardioRpe','cardioValid','cardioSymptoms','cardioStopReason','cardioNotes'],reasonName:'cardioSkipReason'});
  ...
  async function completeIri(){
    ...
    if(!check.ok){const first=IRI_FIRST_SESSION_STEPS.find((step)=>check.byStep[step]?.length)||'revision';setIriStep(form,IRI_FIRST_SESSION_STEPS.indexOf(first));showStepValidation(form,first,check.byStep[first]);focusIriValidationError(form,check.byStep[first]);throw new Error(`M26_IRI_FIRST_SESSION_INVALID:${check.errors.join(',')}`);}
  ...
  async function generateIriReport(variant){
    requireCoach();const form=root.querySelector?.('[data-workflow-form="iri"]');if(!form)throw new Error('M26_IRI_FORM_REQUIRED');const confirmedRecord=currentIriRecord(form);if(!recordBody(confirmedRecord)?.firstSessionCompletedAt)throw new Error('M26_IRI_REPORT_REQUIRES_CONFIRMATION');assertIriRawRanges(form);const draft=iriDraft(form);const check=validateFirstSessionDraft(draft);if(!check.ok){const first=IRI_FIRST_SESSION_STEPS.find((step)=>check.byStep[step]?.length)||'revision';setIriStep(form,IRI_FIRST_SESSION_STEPS.indexOf(first));showStepValidation(form,first,check.byStep[first]);throw new Error(`M26_IRI_FIRST_SESSION_INVALID:${check.errors.join(',')}`);}const result=openIriReportPrint({...reportContext(draft),variant});status(root,'iri',variant==='client'?'Informe Cliente preparado para guardar como PDF.':'Informe Coach / Admin preparado para guardar como PDF.','success');return result;
  }
  ...
  function onInput(event){
    ...
    const appointmentForm=event.target.closest?.('[data-workflow-form="appointment"]');if(appointmentForm){clearStatus(root,'appointment');syncAppointmentFormState(appointmentForm,root);return;}
  ...
  function onChange(event){const clientControl=event.target.closest?.('[data-client-filter],[data-client-sort]');if(clientControl){updateClientList();return;}const filter=event.target.closest?.('[data-library-filter]');if(filter){updateLibrary();return;}const iriForm=event.target.closest?.('[data-workflow-form="iri"]');if(!iriForm)return;computed(iriForm);queueIriSave();}
