...
export function createWorkflowController({
  ...
  function updateClientList(queryOverride=null){const query=String(queryOverride===null?(root.querySelector?.('[data-client-search]')?.value||''):queryOverride).trim();const filters=clientFilterState(root);const sort=String(root.querySelector?.('[data-client-sort]')?.value||'priority');const grid=root.querySelector?.('[data-client-grid]');const cards=[...(root.querySelectorAll?.('[data-client-text]')||[])];const documents=cards.map((card,index)=>({id:String(index),name:String(card.dataset?.clientName||''),text:String(card.getAttribute?.('data-client-text')||''),iri:String(card.getAttribute?.('data-client-iri')||''),modality:String(card.getAttribute?.('data-client-modality')||''),stage:String(card.getAttribute?.('data-client-stage')||''),priority:Number(card.dataset?.clientPriority||99)}));const ranked=rankCoachClientDocuments(documents,{query,filters,sort});const visibleIds=new Set(ranked.map((item)=>item.id));for(let index=0;index<cards.length;index++)cards[index].hidden=!visibleIds.has(String(index));if(grid){for(const item of ranked)grid.appendChild(cards[Number(item.id)]);for(let index=0;index<cards.length;index++){if(!visibleIds.has(String(index)))grid.appendChild(cards[index]);}}const visible=ranked.length;const node=root.querySelector?.('[data-client-search-status]');if(node){const hasFilters=Boolean(filters.iri||filters.modality||filters.stage);node.textContent=query&&!hasFilters?`${visible} ${visible===1?'cliente encontrado':'clientes encontrados'} con búsqueda tolerante.`:`${visible} ${visible===1?'cliente visible':'clientes visibles'} con los filtros actuales.`;}return visible;}
