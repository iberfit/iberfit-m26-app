...
const DEFAULT_FUSE_OPTIONS=Object.freeze({includeScore:true,ignoreLocation:true,threshold:0.36,minMatchCharLength:2});

function text(value,max=300){return String(value??'').trim().slice(0,max);}

export function foldCoachSearch(value){
  return String(value??'').normalize('NFD').replace(/[\u0300-\u036f]/gu,'').toLowerCase().replace(/\s+/gu,' ').trim();
}

function uniqueBy(items,keyOf){
  const seen=new Set();const out=[];
  for(const item of items){const key=keyOf(item);if(!key||seen.has(key))continue;seen.add(key);out.push(item);}
  return out;
}

function safePriority(value){const number=Number(value);return Number.isFinite(number)?number:99;}

export function normalizeClientSearchDocument(input={},index=0){
  return Object.freeze({
    id:text(input.id??index,160)||String(index),name:text(input.name,180),text:foldCoachSearch(input.text),
    iri:foldCoachSearch(input.iri),modality:foldCoachSearch(input.modality),stage:foldCoachSearch(input.stage),
    priority:safePriority(input.priority),index:Number.isFinite(Number(index))?Number(index):0,
  });
}

export function rankCoachClientDocuments(documents=[],{query='',filters={},sort='priority'}={}){
  const normalized=(Array.isArray(documents)?documents:[]).map((item,index)=>normalizeClientSearchDocument(item,index));
  const normalizedFilters={iri:foldCoachSearch(filters?.iri),modality:foldCoachSearch(filters?.modality),stage:foldCoachSearch(filters?.stage)};
  ...
  if(q){
    const fuse=new Fuse(filtered,{...DEFAULT_FUSE_OPTIONS,keys:[{name:'name',weight:0.44},{name:'text',weight:0.56}]});
    return Object.freeze(fuse.search(q).map((result)=>result.item));
  }
  const ordered=[...filtered].sort((a,b)=>sort==='name'
...
export function rankCoachCommandEntries(entries=[],query='',{limit=12}={}){
  const safeEntries=Array.isArray(entries)?entries:[];const q=foldCoachSearch(query);const safeLimit=Math.max(1,Number(limit)||12);
  if(!q)return Object.freeze(safeEntries.slice(0,safeLimit));
  const fuse=new Fuse(safeEntries,{...DEFAULT_FUSE_OPTIONS,threshold:0.4,keys:[{name:'label',weight:0.56},{name:'keywords',weight:0.32},{name:'group',weight:0.12}]});
  return Object.freeze(fuse.search(q,{limit:safeLimit}).map((result)=>result.item));
}
...
export function normalizeCoachSavedView(input={}){
  const name=text(input.name,60);if(!name)throw new Error('M26_COACH_SAVED_VIEW_NAME_REQUIRED');
  return Object.freeze({
    id:text(input.id,120)||`view-${Date.now()}`,name,query:text(input.query,160),iri:text(input.iri,40),modality:text(input.modality,60),stage:text(input.stage,60),
    sort:['priority','name'].includes(String(input.sort||''))?String(input.sort):'priority',updatedAt:text(input.updatedAt,60)||new Date().toISOString(),
  });
}
...
function currentClientFilterState(root){
  ...
  return {query:value('[data-client-search]'),iri:value('[data-client-filter="iri"]'),modality:value('[data-client-filter="modality"]'),stage:value('[data-client-filter="stage"]'),sort:value('[data-client-sort]')||'priority'};
}
...
export function createCoachProductivityController({root,store,ownerId,storage=globalThis.localStorage}={}){
  ...
  function commandAreas(){
    const items=[];
    for(const node of root.querySelectorAll?.('[data-m26-area]')||[]){const area=text(node.getAttribute?.('data-m26-area'),100);const label=text(node.textContent,160);if(area&&label)items.push({area,label});}
  ...
  function renderPalette(query=''){
    ...
    setHtmlIfChanged(host,results.length?results.map(renderCommandResult).join(''):'<p class="m26-coach-command-empty">Sin coincidencias dentro de tu alcance actual.</p>');
  ...
  function savedViewOptions(){
    const select=root.querySelector?.('[data-coach-saved-view]');if(!select)return;
    const current=String(select.value||'');const views=workspace().savedViews;
    setHtmlIfChanged(select,'<option value="">Seleccionar vista…</option>'+views.map((item)=>`<option value="${escapeHtml(item.id)}">${escapeHtml(item.name)}</option>`).join(''));
    if(views.some((item)=>item.id===current))select.value=current;
  }

  function renderRecents(){
    const host=root.querySelector?.('[data-coach-recents]');if(!host)return;
    const state=store.getState();const byId=new Map((state?.collections?.clients||[]).map((client)=>[String(client.id),client]));
    const recentClients=workspace().recents.map((id)=>byId.get(id)).filter(Boolean);
    setHtmlIfChanged(host,recentClients.length
  ...
  function rememberClient(clientId){
    const id=text(clientId,160);if(!id)return;const state=workspace();const recents=[id,...state.recents.filter((item)=>item!==id)].slice(0,COACH_PRODUCTIVITY_MAX_RECENTS);
    persist({...state,recents});renderRecents();
  }

  function saveView(){
    const name=text(root.querySelector?.('[data-coach-view-name]')?.value,60);if(!name){status('Escribe un nombre para guardar esta vista.','error');return false;}
    const filters=currentClientFilterState(root);const current=workspace();const existing=current.savedViews.find((item)=>foldCoachSearch(item.name)===foldCoachSearch(name));const saved=normalizeCoachSavedView({...filters,name,id:existing?.id});
    const savedViews=[saved,...current.savedViews.filter((item)=>item.id!==saved.id)].slice(0,COACH_PRODUCTIVITY_MAX_SAVED_VIEWS);
    if(!persist({...current,savedViews})){status('No fue posible guardar la vista en este dispositivo.','error');return false;}
    const nameInput=root.querySelector?.('[data-coach-view-name]');if(nameInput)nameInput.value='';savedViewOptions();const select=root.querySelector?.('[data-coach-saved-view]');if(select)select.value=saved.id;status(`Vista “${saved.name}” guardada en este dispositivo.`,'success');return true;
  }

  function applyView(id){
    const view=workspace().savedViews.find((item)=>item.id===String(id||''));if(!view)return false;
    for(const [selector,value] of [['[data-client-search]',view.query],['[data-client-filter="iri"]',view.iri],['[data-client-filter="modality"]',view.modality],['[data-client-filter="stage"]',view.stage],['[data-client-sort]',view.sort]]){const node=root.querySelector?.(selector);if(node)node.value=value;}
    dispatchInput(root.querySelector?.('[data-client-search]'));status(`Vista “${view.name}” aplicada.`,'success');return true;
  }

  function deleteView(){
    const select=root.querySelector?.('[data-coach-saved-view]');const id=String(select?.value||'');if(!id){status('Selecciona una vista guardada para eliminarla.','error');return false;}
    const current=workspace();const target=current.savedViews.find((item)=>item.id===id);persist({...current,savedViews:current.savedViews.filter((item)=>item.id!==id)});savedViewOptions();if(select)select.value='';status(target?`Vista “${target.name}” eliminada.`:'Vista eliminada.','success');return true;
  }

  function onClick(event){
    const open=event.target.closest?.('[data-coach-command-open]');if(open){event.preventDefault?.();openPalette();return;}
    const close=event.target.closest?.('[data-coach-command-close]');if(close){event.preventDefault?.();closePalette();return;}
  ...
  function onKeydown(event){
    if(!enabled())return;const key=String(event.key||'').toLowerCase();
    if(key==='k'&&(event.ctrlKey||event.metaKey)&&!event.altKey){event.preventDefault?.();openPalette();return;}
    const palette=root.querySelector?.('[data-coach-command-palette]');if(!palette||palette.hidden)return;
    if(key==='escape'){event.preventDefault?.();closePalette();return;}
    if(key==='enter'&&event.target.closest?.('[data-coach-command-search]')){const first=root.querySelector?.('[data-coach-command-result]');if(first){event.preventDefault?.();first.click?.();}}
  }

  return Object.freeze({
    mount(){
      if(mounted)return;root.addEventListener('click',onClick);root.addEventListener('input',onInput);root.addEventListener('change',onChange);root.addEventListener('m26:shell-rendered',onShellRendered);globalThis.addEventListener?.('keydown',onKeydown);
      if(typeof store.subscribe==='function')unsubscribeStore=store.subscribe(queueHydrate);
      queueHydrate();mounted=true;
    },
    destroy(){
