...
function applyRules(value,language,translatePart){
  ...
  if((m=value.match(/^(\d+) sesión(?:es)? publicada(?:s)?$/u)))return pick(language,`${m[1]} published session${m[1]==='1'?'':'s'}`,`${m[1]} séance${m[1]==='1'?'':'s'} publiée${m[1]==='1'?'':'s'}`,`${m[1]} ${m[1]==='1'?'sessão publicada':'sessões publicadas'}`);
