...
function hasObjectiveMeasurement(value){if(finite(value))return Number(value)>=0;if(!value||typeof value!=='object')return false;return Object.values(value).some(hasObjectiveMeasurement);}
...
export function validateIriDraft(draft={}){
  ...
  if(!draft.strengthPatterns || typeof draft.strengthPatterns!=='object' || !hasObjectiveMeasurement(draft.strengthPatterns)) errors.push('strengthPatterns');
  if(!draft.bodyComposition || typeof draft.bodyComposition!=='object' || !hasObjectiveMeasurement(draft.bodyComposition)) errors.push('bodyComposition');
