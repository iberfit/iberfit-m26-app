...
function clientIriExternalReportComplement(draft,report){
  if(!report||report.visibleToClient!==true||clean(report.assessmentId,80)!==clean(draft.assessmentId,80))return '';
  const href=iriExternalReportAppUrl(draft.assessmentId);
...
function clientPages(draft,context){
  const {clientName,coachName,logoUrl,externalReport}=context;
...
</div><div class="summary-band"><div><span>Confianza de la evaluación</span><strong>${completion.percent===100&&d.reviewAccepted?'Alta':'En revisión'}</strong></div><div><span>Lectura integrada</span><strong>Sin puntuación global</strong></div><div><span>Próxima revisión</span><strong>${escapeHtml(dateLabel(d.reevaluationDate,'Por definir'))}</strong></div></div>`}));
...
function coachPages(draft,context){
  ...
  const coachCardioContent=c.skipped
    ?`${card('Prueba no realizada',`<div class="not-evaluated-panel"><span>NO EVALUADO</span><h3>Sin medición cardiorrespiratoria</h3><p>${escapeHtml(label(c.skipReason,'Motivo no registrado'))}</p></div>`,'highlight')}${card('Trazabilidad de la ausencia',`<div class="mini-list">${row('Estado','No evaluado')}${row('Motivo',c.skipReason)}${row('FC final','No calculada')}${row('ΔFC al minuto','No calculada')}${row('Baremo','No aplicado')}${row('Clasificación','No emitida')}</div>`,'soft')}`
...
export function buildIriReportHtml({draft,variant='client',clientName='Cliente IBERFIT',coachName='Coach IBERFIT',clientId='',logoUrl='/public/isotipo-iberfit.png',stylesheetHref='',externalReport=null}={}){
  if(!draft||!['client','coach'].includes(variant))throw new Error('M26_IRI_REPORT_DOCUMENT_INVALID');
  const context={clientName:clean(clientName,160)||'Cliente IBERFIT',coachName:clean(coachName,160)||'Coach IBERFIT',clientId:clean(clientId,200),logoUrl,externalReport};
...
function reportStylesheetUrl(locationLike=globalThis.location){
  ...
  const url=new URL('/m26/iri-report.css?v=m26-rc36-canary-v10',origin);
...
export function openIriReportPrint({draft,variant='client',clientName,coachName,clientId,logoUrl,externalReport=null,printTarget=null,openWindow=globalThis.open,locationLike=globalThis.location}={}){
  ...
  const html=buildIriReportHtml({draft,variant,clientName,coachName,clientId,logoUrl,stylesheetHref,externalReport});
...
export const __iriReportInternals=Object.freeze({escapeHtml,clean,label,number,dateLabel,heartRateChart,coverageScore,clientIriExternalReportComplement,REPORT_CSS,PREMIUM_RC36_CSS,REPORT_DYNAMIC_CSS,REPORT_STYLESHEET,REPORT_FIT_LEVELS,widthClass,percentWidthClass,reportStylesheetUrl,directIriReportHtml,reportPageNumber,reportPageContentFits,fitLevelForRatio,fitReportPages,reportLayoutReady,waitForReportAssets,bindDirectIriReportWindow,rawDataPages});
