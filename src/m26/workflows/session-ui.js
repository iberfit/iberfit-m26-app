...
export function renderGuidedExecution({execution,session,catalog,actionState,mediaMap,role='client',exerciseMemoryFor=null}={}){
  ...
  if(execution.status==='completed'){
    ...
    const completionClientId=e(execution.clientId||session.clientId||'');
    const completedPrivateNoteAction=isCoach
      ?`<button type="button" data-m26-coach-action="true" data-m26-client-id="${completionClientId}" data-m26-target-area="notas">Añadir nota privada</button>`
      :'';
    const completedProgressAction=isCoach
      ?`<button type="button" class="m26-primary-action" data-m26-coach-action="true" data-m26-client-id="${completionClientId}" data-m26-target-area="expediente">${e(progressActionLabel)}</button>`
    ...
    const completedActions=confirmed
      ?`<div class="m26-session-live-actions"><button type="button" data-session-action="exit-session">Volver a sesiones</button>${completedPrivateNoteAction}${completedProgressAction}</div>`
      :`<button type="button" class="m26-primary-action" data-session-action="exit-session">Volver a sesiones</button>`;
