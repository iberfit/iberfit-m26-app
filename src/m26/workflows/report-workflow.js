...
function premiumEvidenceSignature(candidate){return JSON.stringify({id:candidate?.id||null,assessmentId:candidate?.assessmentId||null,periodStart:candidate?.periodStart||null,periodEnd:candidate?.periodEnd||null,evidence:(candidate?.evidence||[]).map((row)=>[row.label,row.text,row.source,row.quality])});}
...
function premiumMetadataFromActiveEditor(draft,scope=globalThis){
  ...
  const candidate=premiumReportByType(context.state,context.clientId,type,{now:new Date()});const expectedSignature=reportFormValue(form,'premiumEvidenceSignature');
  const sameEvidence=Boolean(candidate?.ready&&candidate.assessmentId===String(draft?.assessmentId||'')&&candidate.periodStart===String(draft?.periodStart||'')&&candidate.periodEnd===String(draft?.periodEnd||'')&&expectedSignature&&premiumEvidenceSignature(candidate)===expectedSignature);
