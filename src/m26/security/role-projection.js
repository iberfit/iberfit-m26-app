...
function projectPublication(key,record){
  ...
  if(key==='trainingCycles'){
    ...
    assign(out,'objective',record,'objective','goal','objetivo','summary','resumen');assign(out,'durationMinutes',record,'durationMinutes','duration_minutes','duracionMinutos','duration');assign(out,'startDate',record,'startDate','start_date');assign(out,'endDate',record,'endDate','end_date');assign(out,'clientVisibilityLevel',record,'clientVisibilityLevel','client_visibility_level');assign(out,'deliveryOwnership',record,'deliveryOwnership','delivery_ownership');assign(out,'deliveryModality',record,'deliveryModality','delivery_modality','modality','modalidad');const visibility=String(value(record,'clientVisibilityLevel','client_visibility_level')||'full').trim().toLowerCase();out.blocks=visibility==='summary_only'?[]:safeSessionBlocks(record);
  }else if(key==='reports'){
...
function projectGeneric(key,record){
  ...
  const mappings={
    ...
    clientProfiles:[
      ...
      ['genderIdentity',['genderIdentity','gender_identity','identidadGenero','identidad_genero']],
      ['pronouns',['pronouns','pronombres']],
      ['email',['email','correo','contactEmail','contact_email']],
      ['phone',['phone','telefono','mobile','movil','contactPhone','contact_phone']],
      ['preferredContactChannel',['preferredContactChannel','preferred_contact_channel','canalContacto','canal_contacto']],
      ['preferredContactTime',['preferredContactTime','preferred_contact_time','horarioContacto','horario_contacto']],
      ['timezone',['timezone','timeZone','zonaHoraria','zona_horaria']],
      ['objective',['objective','objetivo','primaryObjective','primary_objective']],
      ['secondaryObjectives',['secondaryObjectives','secondary_objectives','objetivosSecundarios','objetivos_secundarios']],
      ['modality',['modality','modalidad']],
      ['trainingAddress',['trainingAddress','training_address','trainingLocation','training_location','address','direccion','direccionEntrenamiento','direccion_entrenamiento']],
      ['commune',['commune','comuna','city','ciudad']],
      ['locationType',['locationType','location_type','tipoLugar','tipo_lugar']],
      ['accessInstructions',['accessInstructions','access_instructions','instruccionesAcceso','instrucciones_acceso']],
      ['preferredSchedule',['preferredSchedule','preferred_schedule','horarioPreferido','horario_preferido']],
      ['sessionDurationMinutes',['sessionDurationMinutes','session_duration_minutes','duracionSesionMinutos','duracion_sesion_minutos']],
      ['weeklyFrequency',['weeklyFrequency','weekly_frequency','frecuenciaSemanal','frecuencia_semanal']],
      ['equipmentAvailable',['equipmentAvailable','equipment_available','equipment','materialDisponible','material_disponible']],
      ['emergencyContactName',['emergencyContactName','emergency_contact_name','contactoEmergenciaNombre','contacto_emergencia_nombre']],
      ['emergencyContactRelation',['emergencyContactRelation','emergency_contact_relation','contactoEmergenciaRelacion','contacto_emergencia_relacion']],
      ['emergencyContactPhone',['emergencyContactPhone','emergency_contact_phone','contactoEmergenciaTelefono','contacto_emergencia_telefono']],
      ['firstName',['firstName','first_name']],
      ['lastName',['lastName','last_name']],
    ],
    clientAccess:[['activatedAt',['activatedAt','activated_at']],['lastAccessAt',['lastAccessAt','last_access_at']]],
    iriAssessments:[['assessmentDate',['assessmentDate','assessment_date','evaluatedAt','evaluated_at']],['evaluatedAt',['evaluatedAt','evaluated_at']],['stepFinalHr',['stepFinalHr','step_final_hr']],['stepOneMinuteHr',['stepOneMinuteHr','step_one_minute_hr']],['pushUps',['pushUps','push_ups']],['chairStand30s',['chairStand30s','chair_stand_30s']],['bodyComposition',['bodyComposition','body_composition']],['strengthPatterns',['strengthPatterns','strength_patterns']]],
    ...
    appointments:[['sessionId',['sessionId','session_id']],['title',['title','titulo','name','nombre']],['startAt',['startAt','start_at','scheduledAt','scheduled_at','date']],['endAt',['endAt','end_at']],['location',['location','ubicacion']],['modality',['modality','modalidad']],['visibleToClient',['visibleToClient','visible_to_client']],['confirmedByClientAt',['confirmedByClientAt','confirmed_by_client_at']],['clientResponse',['clientResponse','client_response']],['changeRequestedAt',['changeRequestedAt','change_requested_at']],['changeRequestReason',['changeRequestReason','change_request_reason']],['changeRequest',['changeRequest','change_request']]],
    ...
    wearableDailySummaries:[['provider',['provider','proveedor']],['date',['date','fecha']],['steps',['steps','pasos']],['activeMinutes',['activeMinutes','active_minutes']],['sleepMinutes',['sleepMinutes','sleep_minutes']],['restingHeartRate',['restingHeartRate','resting_heart_rate']],['hrvMs',['hrvMs','hrv_ms']],['vfcMethod',['vfcMethod','vfc_method','hrvMethod','hrv_method']],['activeEnergyKcal',['activeEnergyKcal','active_energy_kcal']],['workoutMinutes',['workoutMinutes','workout_minutes']],['quality',['quality','calidad']],['sourceUpdatedAt',['sourceUpdatedAt','source_updated_at']]],
...
export function projectIdentityForRole(user){const role=roleOf(user);const id=safeId(user?.id);if(!role||!id)throw new Error('M26_IDENTITY_REQUIRED');const out={id,role};if(role!=='client'){const authorizedSource=[user?.authorizedRoles,user?.authorized_roles,user?.roles].find(Array.isArray)||[];const authorizedRoles=[...new Set([...authorizedSource,user?.role].map((item)=>ROLE_ALIASES[String(item||'').trim().toLowerCase()]||null).filter(Boolean))];out.authorizedRoles=authorizedRoles;out.roleChoiceConfirmed=user?.roleChoiceConfirmed===true||user?.role_choice_confirmed===true;}if(role==='client'){const clientId=safeId(user?.clientId||user?.client_id);if(!clientId)throw new Error('M26_CLIENT_IDENTITY_REQUIRED');out.clientId=clientId;}for(const key of ['name','displayName','firstName','lastName','email','status','lastAccessAt']){const source=key==='displayName'?(user?.displayName??user?.display_name):key==='firstName'?(user?.firstName??user?.first_name):key==='lastName'?(user?.lastName??user?.last_name):user?.[key];const safe=cleanScalar(source,key==='email'?254:160);if(safe)out[key]=safe;}return Object.freeze(out);}
export function projectEnvironmentForRole(environment,user){if(roleOf(user)!=='client')return structuredClone(environment??null);if(typeof environment==='string')return cleanScalar(environment,80);const source=environment&&typeof environment==='object'&&!Array.isArray(environment)?environment:{};const out={};for(const key of ['name','mode','reason','version']){const safe=cleanScalar(source[key],100);if(safe)out[key]=safe;}for(const key of ['commandRegistry','installedCommands']){const registry=registryForClient(source[key]);if(registry.length)out[key]=Object.freeze(registry);}if(source.rc39&&typeof source.rc39==='object'&&!Array.isArray(source.rc39)){out.rc39=Object.freeze({authorizedRoles:source.rc39.authorizedRoles===true,appointmentChangeRequests:source.rc39.appointmentChangeRequests===true});}return Object.freeze(out);}
