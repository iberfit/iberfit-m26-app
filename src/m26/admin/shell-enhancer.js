import {renderAdminMediaReviewRoute} from './media-review.js';
...
function injectMediaReview(markup,vm){
  if(vm?.activeArea!=='admin-media-review')return markup;
  const content=renderAdminMediaReviewRoute();
  return String(markup||'').replace(
    /(<main id="m26-main"[^>]*>)[\s\S]*?(<\/main>)/u,
    `$1${content}$2`,
  );
}
export function enhanceAdminShellMarkup(markup,vm){
  ...
  return injectMediaReview(enhanced,vm);
}

export const __adminShellEnhancerInternals=Object.freeze({injectMediaReview});
