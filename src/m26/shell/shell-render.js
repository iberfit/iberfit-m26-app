  ...
  return `<div class="m26-shell" data-m26-role="${escapeHtml(vm.identity.role)}"><a class="m26-skip-link" href="#m26-main">Saltar al contenido</a>
    ...
    <section class="m26-workspace">
      <header class="m26-topbar">
        ...
        <div class="m26-topbar-actions">${productivity.launcher}${clientSelector(vm)}${operationStatus(vm.operations)}${(vm.identity.role==='client'||vm.selectedClient)?`<button type="button" class="m26-icon-button" data-m26-area="retos">Retos</button>`:''}${vm.identity.role==='admin'?'<button type="button" class="m26-icon-button" data-m26-area="admin-configuracion">Configuración</button>':'<button type="button" class="m26-icon-button" data-m26-area="ajustes">Ajustes</button>'}<button type="button" class="m26-icon-button" data-m26-action="logout">Cerrar sesión</button><button type="button" class="m26-danger-action" data-m26-action="logout-clear-device">Cerrar sesión y borrar datos de este dispositivo</button></div>
      </header>
