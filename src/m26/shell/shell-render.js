...
function shellStyles(){
  ...
  .m26-sidebar-logout{width:100%;margin-top:.35rem;padding:.55rem .72rem;border:0;border-radius:.75rem;color:rgba(247,241,228,.62);background:transparent;text-align:left;font:inherit}.m26-sidebar-logout:hover,.m26-sidebar-logout:focus-visible{color:var(--m26-v2-cream);background:rgba(255,255,255,.045)}
  ...
  @media(max-width:900px){.m26-sidebar-footer{display:none}.m26-topbar{min-height:3.9rem}.m26-topbar-actions{max-width:66%}}
...
function renderM26ShellBase(vm, routeMarkup = '') {
  ...
  const mobileAccountAction=`<div class="m26-mobile-more-account"><button type="button" class="m26-nav-item" data-m26-action="logout">${escapeHtml(tx('common.logout','Cerrar sesión'))}</button></div>`;
  const mobileMore = moreMobileItems.length ? `<details class="m26-mobile-more${mobileMoreActive?' is-active':''}"${mobileMoreActive?' data-m26-more-active="true"':''}><summary>${escapeHtml(tx('common.more','Más'))}</summary><div class="m26-mobile-more-menu">${moreMobileItems.map((item) => navItem(item, vm.activeArea)).join('')}${mobileAccountAction}</div></details>` : '';
