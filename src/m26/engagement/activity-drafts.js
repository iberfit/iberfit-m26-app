...
export function normalizeCheckinDraft(input={}){const value={energy:score(input.energy),sleep:score(input.sleep),stress:score(input.stress),pain:score(input.pain),notes:String(input.notes||'').trim().slice(0,1000),recordedAt:iso(input.recordedAt)};if(provided(input.fatigue))value.fatigue=score(input.fatigue);if(provided(input.motivation))value.motivation=score(input.motivation);return Object.freeze(value);}
export function validateCheckinDraft(input={}){const value=normalizeCheckinDraft(input);const errors=[];for(const key of ['energy','sleep','stress','pain'])if(value[key]===null)errors.push(`${key.toUpperCase()}_REQUIRED`);if(provided(input.fatigue)&&value.fatigue===null)errors.push('FATIGUE_INVALID');if(provided(input.motivation)&&value.motivation===null)errors.push('MOTIVATION_INVALID');if(!value.recordedAt)errors.push('RECORDED_AT_INVALID');return Object.freeze({ok:errors.length===0,errors:Object.freeze(errors),value});}
