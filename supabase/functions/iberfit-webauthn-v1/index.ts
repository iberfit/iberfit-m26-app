...
const RP_ID='m26-canary.iberfit.cl';
const ORIGIN='https://m26-canary.iberfit.cl';
const FUNCTION_VERSION='rc65c1-free-webauthn-v1';
...
function fail(status,code){return response(status,{ok:false,code:safeCode(code)});}
...
async function createChallenge(admin,{userId,sessionId,ceremony,challenge}){
  const expiresAt=new Date(Date.now()+CHALLENGE_TTL_MS).toISOString();
  const {data,error}=await admin.from('iberfit_webauthn_challenges_v1').insert({user_id:userId,session_id:sessionId,ceremony,challenge,origin:ORIGIN,expires_at:expiresAt}).select('id').single();
  if(error||!data?.id)throw new Error('M26_WEBAUTHN_CHALLENGE_STORE_FAILED');
  return String(data.id);
}
async function consumeChallenge(admin,{challengeId,userId,sessionId,ceremony}){
...
async function main(req){
  if(req.headers.get('origin')!==ORIGIN)return fail(403,'M26_WEBAUTHN_ORIGIN_FORBIDDEN');
  if(req.method==='OPTIONS')return new Response(null,{status:204,headers:corsHeaders()});
  if(req.method!=='POST')return fail(405,'M26_METHOD_NOT_ALLOWED');
  ...
  if(!authorization.startsWith('Bearer '))return fail(401,'M26_AUTH_REQUIRED');
  const token=authorization.slice(7).trim();if(!token||token.length>16384)return fail(401,'M26_AUTH_REQUIRED');
  ...
  const user=userData?.user;if(userError||!user?.id)return fail(401,'M26_AUTH_INVALID');
  const claims=decodeJwtPayload(token);const sessionId=String(claims?.session_id||'');
  if(!UUID_PATTERN.test(sessionId)||String(claims?.sub||'')!==String(user.id))return fail(401,'M26_AUTH_SESSION_INVALID');

  const {data:context,error:contextError}=await userClient.rpc('iberfit_application_context_v14');
  if(contextError||!context?.ok)return fail(403,'M26_APPLICATION_CONTEXT_REQUIRED');
  ...
  if(!privilegedRole)return fail(403,'M26_PRIVILEGED_ROLE_REQUIRED');
  ...
  if(action==='registration-options'){
    ...
    const challengeId=await createChallenge(admin,{userId:user.id,sessionId,ceremony:'registration',challenge:options.challenge});
    return response(200,{ok:true,challengeId,type:'create',credentialOptions:{publicKey:options},privilegedRole,version:FUNCTION_VERSION});
  }

  if(action==='registration-verify'){
    const credential=credentialResponse(body.credentialResponse);
    const expectedChallenge=await consumeChallenge(admin,{challengeId:String(body.challengeId||''),userId:user.id,sessionId,ceremony:'registration'});
    let verification;try{verification=await verifyRegistrationResponse({response:credential,expectedChallenge,expectedOrigin:ORIGIN,expectedRPID:RP_ID,requireUserVerification:true});}catch{return fail(400,'M26_WEBAUTHN_REGISTRATION_VERIFICATION_FAILED');}
    if(!verification?.verified||!verification.registrationInfo)return fail(400,'M26_WEBAUTHN_REGISTRATION_NOT_VERIFIED');
    ...
    if(insertError)return fail(409,'M26_WEBAUTHN_CREDENTIAL_ALREADY_REGISTERED');
    const expiresAt=await writeAssurance(admin,{userId:user.id,sessionId,credentialId:String(registered.id)});
    return response(200,{ok:true,verified:true,user:{id:user.id,email:user.email},privilegedRole,expiresAt,version:FUNCTION_VERSION});
  }
  ...
  if(action==='authentication-options'){
    ...
    if(readError)throw new Error('M26_WEBAUTHN_CREDENTIAL_READ_FAILED');if(!credentials?.length)return fail(409,'M26_WEBAUTHN_NOT_ENROLLED');
    ...
    const challengeId=await createChallenge(admin,{userId:user.id,sessionId,ceremony:'authentication',challenge:options.challenge});
    return response(200,{ok:true,challengeId,type:'request',credentialOptions:{publicKey:options},privilegedRole,version:FUNCTION_VERSION});
  }

  if(action==='authentication-verify'){
    const credential=credentialResponse(body.credentialResponse);
    const expectedChallenge=await consumeChallenge(admin,{challengeId:String(body.challengeId||''),userId:user.id,sessionId,ceremony:'authentication'});
    ...
    if(storedError||!stored)return fail(404,'M26_WEBAUTHN_CREDENTIAL_NOT_FOUND');
    ...
    let verification;try{verification=await verifyAuthenticationResponse({response:credential,expectedChallenge,expectedOrigin:ORIGIN,expectedRPID:RP_ID,requireUserVerification:true,credential:{id:String(stored.credential_id),publicKey:base64UrlToBytes(stored.public_key_b64),counter,transports:normalizedTransports(stored.transports)}});}catch{return fail(400,'M26_WEBAUTHN_AUTHENTICATION_VERIFICATION_FAILED');}
    if(!verification?.verified)return fail(400,'M26_WEBAUTHN_AUTHENTICATION_NOT_VERIFIED');
  ...
  return fail(400,'M26_WEBAUTHN_ACTION_INVALID');
}

Deno.serve(async(req)=>{try{return await main(req);}catch(error){console.error(`[IBERFIT:${FUNCTION_VERSION}] ${safeCode(error?.message)}`);return fail(500,error?.message);}});
