-- IBERFIT · ADMIN client deletion · PL/pgSQL static-analysis hardening.
-- Preserve the reviewed delete/history semantics while iterating constant table sets
-- relationally so plpgsql_check resolves each table identifier independently.
...
declare
  ...
  v_detach_history_tables constant text[] := array[
    'beta_incidents_v16','beta_participants_v16','consent_acceptances_v17',
    'data_subject_requests_v17','device_conflict_trials','incident_register_v17'
  ];
begin
  ...
  -- Fail closed for any future restricted client relation that has not been explicitly reviewed.
  for v_ref in
    ...
    where tc.constraint_type='FOREIGN KEY' and tc.table_schema='public' and kcu.column_name='client_id'
      ...
      and tc.table_name<>all(v_detach_history_tables)
  loop
  ...
  -- Preserve reviewed governance/history rows, but sever the personal client relation.
  for v_table in select unnest(v_detach_history_tables) loop
    if to_regclass(format('public.%I',v_table)) is not null and exists(
      select 1 from information_schema.columns c
      where c.table_schema='public' and c.table_name=v_table and c.column_name='client_id' and c.is_nullable='YES'
    ) then
      execute format('update public.%I set client_id=null where client_id=$1',v_table) using v_client_id;
      get diagnostics v_rows=row_count;
      v_detached:=v_detached+coalesce(v_rows,0);
    else
      raise exception 'IBERFIT_CLIENT_DELETE_HISTORY_SCHEMA_DRIFT:%',v_table using errcode='23503';
    end if;
  end loop;
  ...
  for v_table in select unnest(v_cleanup_tables) loop
    if to_regclass(format('public.%I',v_table)) is not null and exists(
  ...
  insert into public.iberfit_admin_audit_events(
    ...
    concat('Cliente eliminado de forma permanente por ADMIN. Motivo: ',left(v_reason,240),
      '. Filas operativas eliminadas: ',v_removed,'. Filas históricas desvinculadas: ',v_detached,
  ...
  v_result:=jsonb_build_object('ok',true,'kind','ack','operationId',v_operation,'commandType',v_type,
    ...
    'explicitRowsRemoved',v_removed,'historicalRowsDetached',v_detached,
    'authIdentityRetained',v_auth_user_id is not null,'auditId',v_audit_id,'serverTime',now());
...
alter function public.iberfit_admin_delete_client_v26(jsonb,jsonb) owner to postgres;
revoke all on function public.iberfit_admin_delete_client_v26(jsonb,jsonb) from public,anon,authenticated;
grant execute on function public.iberfit_admin_delete_client_v26(jsonb,jsonb) to service_role;

comment on function public.iberfit_admin_delete_client_v26(jsonb,jsonb) is
  'IBERFIT destructive ADMIN client deletion; reviewed history is detached, operational rows are removed, unknown protected references fail closed, and constant table sets use lint-safe relational iteration.';
...
begin
  select
    ...
    regexp_replace(lower(pg_get_functiondef(p.oid)), '\s+', '', 'g'),
    p.prosecdef,
    pg_get_userbyid(p.proowner),
    p.proconfig
  into v_src,v_compact,v_security_definer,v_owner,v_config
  from pg_proc p
  where p.oid='public.iberfit_admin_delete_client_v26(jsonb,jsonb)'::regprocedure;

  if v_src is null then
    raise exception 'IBERFIT_ADMIN_CLIENT_DELETE_POSTCHECK_MISSING';
  end if;
  if position('forv_tableinselectunnest(v_detach_history_tables)loop' in v_compact)=0 then
    raise exception 'IBERFIT_ADMIN_CLIENT_DELETE_POSTCHECK_DETACH_ITERATION';
  end if;
  if position('forv_tableinselectunnest(v_cleanup_tables)loop' in v_compact)=0 then
    raise exception 'IBERFIT_ADMIN_CLIENT_DELETE_POSTCHECK_CLEANUP_ITERATION';
  end if;
  if position('foreachv_tableinarray' in v_compact)>0 then
    raise exception 'IBERFIT_ADMIN_CLIENT_DELETE_POSTCHECK_FOREACH_REGRESSION';
  end if;
  if not v_security_definer or v_owner<>'postgres' or not (v_config @> array['search_path=""']::text[]) then
    raise exception 'IBERFIT_ADMIN_CLIENT_DELETE_POSTCHECK_SECURITY';
  end if;
  if has_function_privilege('anon','public.iberfit_admin_delete_client_v26(jsonb,jsonb)','EXECUTE')
     or has_function_privilege('authenticated','public.iberfit_admin_delete_client_v26(jsonb,jsonb)','EXECUTE')
     or not has_function_privilege('service_role','public.iberfit_admin_delete_client_v26(jsonb,jsonb)','EXECUTE') then
    raise exception 'IBERFIT_ADMIN_CLIENT_DELETE_POSTCHECK_ACL';
  end if;
end
$postcheck$;
