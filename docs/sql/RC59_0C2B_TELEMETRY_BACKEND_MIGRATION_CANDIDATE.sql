...
-- TELEMETRY BACKEND MIGRATION EXECUTABLE CANDIDATE
-- NOT YET APPROVED FOR REMOTE APPLY.
--
-- This candidate remains deliberately outside supabase/migrations/.
-- RC59.0C2A canonical preflight + authorization drift correction are prerequisites.
-- Manual execution additionally requires a session-local authorization flag.
--
-- Required before execution in the SAME transaction/session:
--   set local iberfit.rc59_0c2_apply_authorized =
--     'RC59_0C2B_AUTHORIZED';

begin;

do $rc59_apply_authorization$
begin
  if current_setting(
    'iberfit.rc59_0c2_apply_authorized',
    true
  ) is distinct from 'RC59_0C2B_AUTHORIZED'
  then
    raise exception 'RC59_0C2B_APPLY_AUTHORIZATION_REQUIRED';
  end if;
end
$rc59_apply_authorization$;
...
begin
  ...
  if to_regprocedure(
    ...
    raise exception 'M26_RC59_AUTH_HELPER_ALREADY_PRESENT';
  end if;

  if to_regprocedure(
    'public.m26_telemetry_json_safe_v59(jsonb)'
  ) is not null then
    raise exception 'M26_RC59_JSON_GUARD_ALREADY_PRESENT';
  end if;

  if to_regprocedure(
    'public.m26_telemetry_event_valid_v59(jsonb,uuid,text,text)'
  ) is not null then
    raise exception 'M26_RC59_EVENT_VALIDATOR_ALREADY_PRESENT';
  end if;

  if to_regprocedure(
    'public.m26_telemetry_import_v59(jsonb)'
  ) is not null then
    raise exception 'M26_RC59_IMPORT_RPC_ALREADY_PRESENT';
  end if;

  if to_regprocedure(
    'public.m26_telemetry_read_page_v59(uuid,timestamp with time zone,integer)'
  ) is not null then
    raise exception 'M26_RC59_READ_RPC_ALREADY_PRESENT';
  end if;

  if to_regprocedure(
    'public.m26_telemetry_delete_own_v59(timestamp with time zone)'
  ) is not null then
    raise exception 'M26_RC59_DELETE_RPC_ALREADY_PRESENT';
  end if;

  if to_regprocedure(
    'public.m26_telemetry_purge_expired_v59()'
  ) is not null then
    raise exception 'M26_RC59_PURGE_RPC_ALREADY_PRESENT';
  end if;
end
$rc59_guard$;
...
create table public.m26_telemetry_events_v59 (
  ...
  check (
    ...
    and (canonical_event ->> 'recordedAt')::timestamptz = recorded_at
    and (canonical_event ->> 'receivedAt')::timestamptz = received_at
  )
);
...
begin
  ...
  if v_force_rls <> 2 then
    raise exception 'M26_RC59_FORCE_RLS_POSTCHECK_FAILED';
  end if;
  ...
  then
    raise exception 'M26_RC59_DIRECT_TABLE_PRIVILEGE_POSTCHECK_FAILED';
  end if;
  ...
  if not has_function_privilege(
    ...
    raise exception 'M26_RC59_IMPORT_EXECUTE_POSTCHECK_FAILED';
  end if;
  ...
  if not has_function_privilege(
    ...
    raise exception 'M26_RC59_READ_EXECUTE_POSTCHECK_FAILED';
  end if;
  ...
  if not has_function_privilege(
    ...
    raise exception 'M26_RC59_DELETE_EXECUTE_POSTCHECK_FAILED';
  end if;
  ...
  if has_function_privilege(
    ...
    raise exception 'M26_RC59_PURGE_EXECUTE_POSTCHECK_FAILED';
  end if;
end
$rc59_postcheck$;
