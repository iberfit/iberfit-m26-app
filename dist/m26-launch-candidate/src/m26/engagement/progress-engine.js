...
function dateOf(record){return first(record,'completedAt','completed_at','endedAt','ended_at','startAt','start_at','scheduledAt','scheduled_at','evaluatedAt','evaluated_at','createdAt','created_at','date','fecha');}
...
function volumeOf(execution){
  return setRows(execution).reduce((total,row)=>{
    ...
    return total+(Number.isFinite(reps)&&Number.isFinite(load)?reps*load:0);
  },0);
}
...
export function progressWindow({now=new Date(),days=28}={}){
  const end=new Date(now);const start=new Date(end);start.setDate(start.getDate()-Math.max(1,Number(days||28)));return {start,end};
}
...
export function computeProgressSummary(state,clientId,{now=new Date(),days=28}={}){
  ...
  const volumes=completedExecutions.map(volumeOf).filter((value)=>value>0);
  const half=Math.max(1,Math.floor(volumes.length/2));
  const olderVolume=average(volumes.slice(0,half));
  const recentVolume=average(volumes.slice(half));
  ...
  const lastExecution=completedExecutions.sort(byDateDesc)[0]||null;
  ...
  return Object.freeze({
    clientId,startAt:start.toISOString(),endAt:end.toISOString(),days,
    ...
    lastExecutionAt:dateOf(lastExecution)||null,dataQuality,
  });
}
...
export function buildProgressTimeline(state,clientId,{now=new Date(),days=90,limit=24}={}){
  ...
  for(const item of forClient(state,'checkins',clientId).map(unwrap))if(within(dateOf(item),start,end)){const values=checkinValues(item);rows.push({kind:'checkin',date:dateOf(item),title:'Check-in',status:'registrado',detail:`Energía ${values.energy??'—'} · Sueño ${values.sleep??'—'} · Estrés ${values.stress??'—'} · Dolor ${values.pain??'—'}`});}
  return rows.sort((a,b)=>(safeDate(b.date)?.getTime()||0)-(safeDate(a.date)?.getTime()||0)).slice(0,limit).map(clone);
}
