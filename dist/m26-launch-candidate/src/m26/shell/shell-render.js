...
function navItem(item, activeArea) {
  ...
  return `<button class="m26-nav-item${active ? ' is-active' : ''}" type="button" data-m26-area="${escapeHtml(item.key)}" aria-current="${active ? 'page' : 'false'}"><span>${escapeHtml(item.label)}</span></button>`;
}
...
function operationStatus(operations) {
  ...
  return `<div class="m26-operation-status is-${escapeHtml(operations.kind)}"><span class="m26-status-dot" aria-hidden="true"></span><span>${escapeHtml(text)}</span></div>`;
}
...
export function renderM26AccessFrame(vm) {
  ...
  return `<main class="m26-access-frame"><section><img src="/public/isotipo-iberfit.png" alt="" class="m26-access-mark"><p class="m26-eyebrow">IBERFIT</p><h1>Entrenamiento personal con criterio</h1><p>Diagnóstico, planificación, control y seguimiento.</p><div class="m26-access-status" role="status">${escapeHtml(state)}</div></section></main>`;
}
...
export function renderM26Shell(vm, routeMarkup = '') {
  ...
  const routeContent = routeMarkup || `<section class="m26-route-placeholder" aria-live="polite"><p class="m26-eyebrow">${escapeHtml(vm.page.label)}</p><h2>${escapeHtml(vm.page.title)}</h2><p>El módulo se conectará a este shell sin alterar identidad, permisos ni contexto de cliente.</p></section>`;
  ...
  return `<div class="m26-shell" data-m26-role="${escapeHtml(vm.identity.role)}">
    ...
    <section class="m26-workspace">
      <header class="m26-topbar">
        <div><p class="m26-eyebrow">${escapeHtml(vm.identity.roleLabel)}</p><h1>${escapeHtml(vm.page.title)}</h1></div>
      ...
      <main id="m26-main" class="m26-main" tabindex="-1">${routeContent}</main>
