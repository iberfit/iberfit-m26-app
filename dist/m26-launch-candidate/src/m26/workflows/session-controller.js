import { addCatalogExercise, addTrainingGroup, closeTrainingGroup, buildPublishSessionCommand } from './session-builder.js';
import {
...
export function dispatchSessionAction({action,draft,execution,session,catalog,payload={},commandBus,appointmentId,sessionRevision=0,online=true,offlinePermit={}}={}){
  switch(action){
    ...
    case 'remove-block': draft.blocks=draft.blocks.filter((b)=>b.id!==payload.blockId); return {kind:'draft',value:draft};
    ...
    case 'publish': {
      ...
      draft.previewAccepted=true;const command=buildPublishSessionCommand(draft,catalog,draft.revision||0);
    ...
    case 'pause': {
      if(!commandBus){pauseExecution(execution);return {kind:'execution',value:execution};}
      const command=buildPauseExecutionCommand(execution,execution.revision||0);
      if(!isOnline(online))return {kind:'queued',value:enqueueAndApply(commandBus,command,()=>pauseExecution(execution),execution)};
      return {kind:'command',value:executeAndApply(commandBus,command,(result)=>{pauseExecution(execution);execution.revision=remoteRevision(result,execution.revision);},execution)};
    }
    case 'resume': {
      if(!commandBus){resumeExecution(execution);return {kind:'execution',value:execution};}
      const command=buildResumeExecutionCommand(execution,execution.revision||0);
      if(!isOnline(online))return {kind:'queued',value:enqueueAndApply(commandBus,command,()=>resumeExecution(execution),execution)};
      return {kind:'command',value:executeAndApply(commandBus,command,(result)=>{resumeExecution(execution);execution.revision=remoteRevision(result,execution.revision);},execution)};
    }
    case 'cancel': {
      const reason=String(payload.reason||'').trim();
      if(!commandBus){cancelExecution(execution,reason);return {kind:'execution',value:execution};}
      const command=buildCancelExecutionCommand(execution,reason,execution.revision||0);
      if(!isOnline(online))return {kind:'queued',value:enqueueAndApply(commandBus,command,()=>cancelExecution(execution,reason),execution)};
      return {kind:'command',value:executeAndApply(commandBus,command,(result)=>{cancelExecution(execution,reason);execution.revision=remoteRevision(result,execution.revision);},execution)};
    }
    case 'finish': {
...
export function createSessionController({root,getContext,render,onError=()=>{}}){if(!root?.addEventListener)throw new Error('M26_SESSION_ROOT_REQUIRED');
  ...
  async function click(event){const button=event.target.closest?.('[data-session-action]');if(!button)return;const action=button.getAttribute('data-session-action');const context=getContext();if(action==='exit-session'){context.onExit?.();return;}const actionState=context.actionState;button.disabled=true;button.setAttribute('aria-busy','true');
    const task=async()=>{const payload={exerciseId:button.getAttribute('data-exercise-id'),blockId:button.getAttribute('data-block-id'),groupType:button.getAttribute('data-group-type'),...fieldValues(root)};if(action==='start'){payload.appointmentId=context.appointmentId;payload.sessionRevision=context.sessionRevision;}if(action==='substitute'){payload.fromExerciseId=button.getAttribute('data-from-exercise-id');payload.toExerciseId=root.querySelector?.('[data-session-substitute]')?.value;payload.reason=root.querySelector?.('[data-session-substitute-reason]')?.value;}if(action==='cancel'){payload.reason=root.querySelector?.('[data-session-cancel-reason]')?.value;}if(action==='finish'){payload.sessionRpe=root.querySelector?.('[data-session-feedback-rpe]')?.value;payload.comment=root.querySelector?.('[data-session-feedback-comment]')?.value;payload.pain=root.querySelector?.('[data-session-feedback-pain]')?.checked;payload.painNotes=root.querySelector?.('[data-session-feedback-pain-notes]')?.value;}const result=dispatchSessionAction({...context,action,payload});return await result.value;};
    try{const outcome=actionState?await runAction(actionState,task):{ok:true,value:await task()};if(!outcome.ok)onError(outcome.error);await persistContext(context);render?.();}catch(error){await persistContext(context).catch(()=>{});onError(error);}finally{button.disabled=false;button.removeAttribute('aria-busy');}}
  function input(event){const search=event.target.closest?.('[data-session-search]');if(search){getContext().setQuery?.(search.value);render?.();}getContext().autosaveDraft?.();}
  return Object.freeze({mount(){root.addEventListener('click',click);root.addEventListener('input',input);},destroy(){root.removeEventListener('click',click);root.removeEventListener('input',input);}});
}
