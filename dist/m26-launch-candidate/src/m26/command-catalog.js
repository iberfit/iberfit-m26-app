...
const rows = [
  ...
  ['SESION_INICIAR','session','INICIAR',['admin','coach','cliente'],false,false,false,true,false,true],
  ['SESION_COMPLETAR','session','COMPLETAR',['admin','coach','sistema'],false,false,false,true,false,true],
  ['SESION_CANCELAR','session','CANCELAR',['admin','coach'],true,false,false,true,false,true],
  ['EJECUCION_INICIAR','session_execution','INICIAR',['admin','coach','cliente'],false,false,false,true,false,true],
  ['EJECUCION_GUARDAR_PROGRESO','session_execution','GUARDAR',['admin','coach','cliente'],false,false,false,false,false,true],
  ['EJECUCION_PAUSAR','session_execution','PAUSAR',['admin','coach','cliente'],false,false,false,true,false,true],
  ['EJECUCION_REANUDAR','session_execution','REANUDAR',['admin','coach','cliente'],false,false,false,true,false,true],
  ['EJECUCION_COMPLETAR','session_execution','COMPLETAR',['admin','coach','cliente'],false,false,false,true,false,true],
  ['EJECUCION_CANCELAR','session_execution','CANCELAR',['admin','coach'],true,false,false,true,false,true],
...
export function validateCommandCatalog(installed = [], expectedRegistry=M26_COMMAND_REGISTRY) {
  const expectedMap=registryMap(expectedRegistry);const normalized = normalizeInstalled(installed);const installedMap = new Map(normalized.map((entry) => [entry.type, entry]));
  const expectedTypes=expectedRegistry.map((entry)=>entry.type);const missing = expectedTypes.filter((type) => !installedMap.has(type));
  const unexpected = [...installedMap.keys()].filter((type) => !expectedMap.has(type));const mismatches = [];
  for (const expected of expectedRegistry) {const actual = installedMap.get(expected.type);if (!actual) continue;
    for (const key of ['entityType', 'eventName', 'requiresReason', 'requiresPreview']) if (actual[key] !== undefined && actual[key] !== expected[key]) mismatches.push({ type: expected.type, field: key, expected: expected[key], actual: actual[key] });
    if (actual.allowedRoles && JSON.stringify([...actual.allowedRoles].map(normalizeRegistryRole).sort()) !== JSON.stringify([...expected.allowedRoles].map(normalizeRegistryRole).sort())) mismatches.push({ type: expected.type, field: 'allowedRoles', expected: expected.allowedRoles, actual: actual.allowedRoles });
    if (actual.enabled === false) mismatches.push({ type: expected.type, field: 'enabled', expected: true, actual: false });
  }
  return { ok: missing.length === 0 && unexpected.length === 0 && mismatches.length === 0, required: expectedTypes.length, installed: installedMap.size, missing, unexpected, mismatches };
}
export function validatedRuntimeRegistry(installed=[]){
  const normalized=normalizeInstalled(installed);const installedMap=new Map(normalized.map((entry)=>[entry.type,entry]));const accepted=[];const rejected=[];
  for(const expected of M26_EXTENDED_COMMAND_REGISTRY){const actual=installedMap.get(expected.type);if(!actual)continue;const check=validateCommandCatalog([actual],[expected]);if(check.ok)accepted.push(expected);else rejected.push(...check.mismatches.map((item)=>({...item,type:expected.type})));}
  const base=validateCommandCatalog(normalized.filter((item)=>M26_COMMAND_TYPES.includes(item.type)),M26_COMMAND_REGISTRY);
