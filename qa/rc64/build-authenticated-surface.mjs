...
if(process.env.M26_PROJECT_REF!==PROJECT_REF)throw new Error('RC64_2B_AUTH_PROJECT_REF_MISMATCH');
if(String(process.env.M26_QA_ONLY).toLowerCase()!=='true')throw new Error('RC64_2B_AUTH_QA_ONLY_REQUIRED');
const url=new URL(String(process.env.M26_SUPABASE_URL||''));
if(url.origin!==expectedOrigin||url.pathname!=='/'||url.search||url.hash)throw new Error('RC64_2B_AUTH_PROJECT_MISMATCH');
...
console.log(JSON.stringify({schema:'iberfit.rc64.2b.auth-current-source-surface.v1',source:'canonical-working-tree',output:'.tmp/rc64-current-surface',runtimeEnabled:true,qaOnly:true,projectRef:PROJECT_REF,credentialsEmbedded:false,backendMutationAllowed:false},null,2));
