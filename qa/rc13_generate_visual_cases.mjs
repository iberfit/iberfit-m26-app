...
const outDir=path.join(__dirname,'rc13_visual_cases');
...
const cases=JSON.parse(fs.readFileSync(path.join(__dirname,'rc13_visual_cases.json'),'utf8'));
...
function stateFor(role,activeArea,scenario='normal'){
  ...
  return createProductionState({
    ...
    canary:{active:true,scope:'allowlist',version:'26.0.0-recovery.13'},selectedClientId:clientId,activeArea,
    collections:{...createProductionState().collections,
      ...
      iriAssessments:[
        ...
        },
        {
          ...
          strengthPatterns:{
            chairStand:{repetitions:15,valid:true},
            push:{variant:'standard',repetitions:10,valid:true},
            trxRow:{repetitions:12,valid:true},
            core:{frontPlankSeconds:45},
          },
          personProfile:{birthDate:'1990-02-03',sexForNorms:'female',email:'cliente.qa@iberfit.cl',phone:'+56 9 5555 0101',modality:'hibrido',trainingAddress:'Las Condes',primaryObjective:'Fuerza, salud y autonomía',weeklyFrequency:3,sessionDurationMinutes:60},
          diagnosis:{strengths:['Buena disposición al entrenamiento'],priorities:['Fuerza tren inferior','Capacidad cardiorrespiratoria'],coachInterpretation:'Punto de partida confirmado para iniciar una progresión individualizada.',trainingImplications:'Priorizar técnica y progresión gradual.',initialPlan:'Ciclo base de ocho semanas con tres sesiones por semana.',recommendedFrequency:'3 sesiones por semana',reevaluationDate:'2026-07-18',reviewAccepted:true},
        },
      ],
...
console.log(`Generated ${cases.length} RC13 visual cases`);
