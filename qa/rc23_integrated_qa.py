...
REPORT=ROOT/'recovery'/'RC23_INTEGRATED_QA_REPORT.json'
ART=ROOT/'qa'/'rc23_integrated_artifacts'
...
def integrated_html(role: str) -> str:
    ...
    bundle=(ROOT/'qa/rc23_bundle.js').read_text(encoding='utf-8')
    return f"""<!doctype html><html lang='es-ES'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><base href='https://qa.invalid/'><style>{css}</style></head><body><div id='app'></div><script>globalThis.__RC23_ROLE__={json.dumps(role)};{AMD_LOADER}</script><script>{bundle}</script><script>require(['qa/rc23_integrated_app']);</script></body></html>"""
...
def run_role(browser,role):
    ...
    command_count=page.evaluate('window.__RC23_QA__.commands.length')
    ...
    result={'role':role,'routes':route_results,'actions':[{'name':n,'ok':ok} for n,ok in actions],'command_count':command_count,'console_errors':console,'page_errors':errors,'screenshot':str(shot.relative_to(ROOT)),'finalMetrics':final_metrics}
    result['ok']=all(r['ok'] for r in route_results) and all(a['ok'] for a in result['actions']) and final_metrics.get('documentLanguage')=='es-ES' and not final_metrics['forbiddenLanguageHits'] and not console and not errors and command_count>= (5 if role=='coach' else 4)
