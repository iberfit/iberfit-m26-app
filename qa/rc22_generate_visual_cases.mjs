...
const outDir=path.join(__dirname,'rc22_visual_cases');
...
const cases=JSON.parse(fs.readFileSync(path.join(__dirname,'rc22_visual_cases.json'),'utf8'));
...
function stateFor(role,activeArea,scenario='normal'){
  ...
  return createProductionState({
    ...
    canary:{active:true,scope:'allowlist',version:'26.0.0-state-performance-ux.22'},selectedClientId:clientId,activeArea,
...
for(const item of cases){
  ...
  const html=`<!doctype html><html lang="es-CL"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><meta name="color-scheme" content="dark"><title>${item.name}</title><style>html,body{margin:0;min-width:320px;min-height:100%;background:#07150f}${css}</style></head><body data-qa-status="pass" data-qa-role="${item.role}" data-qa-route="${item.route}" data-qa-scenario="${item.scenario}">${markup}<script>window.__RC22_QA__={wearables:true,zeroCostOnly:true};</script></body></html>`;
  fs.writeFileSync(path.join(outDir,`${item.name}.html`),html);
}
console.log(`Generated ${cases.length} RC22 visual cases`);
