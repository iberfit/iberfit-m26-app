...
async function loginCompleteWebAuthnAndChooseAdmin(page,email,password){
  ...
  await expect(page.locator('.m26-shell[data-m26-role="client"]:not([inert])'),'No Client shell may be active before explicit app choice').toHaveCount(0);
  await expect(page.locator('.m26-shell[data-m26-role="admin"]:not([inert])'),'No Admin shell may be active before explicit app choice').toHaveCount(0);
