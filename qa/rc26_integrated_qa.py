...
REPORT=ROOT/'recovery'/'RC26_INTEGRATED_QA_REPORT.json'
ART=ROOT/'qa'/'rc26_integrated_artifacts'
...
def integrated_html(role: str) -> str:
    ...
    bundle=(ROOT/'qa/rc26_bundle.js').read_text(encoding='utf-8')
    return f"""<!doctype html><html lang='es-ES'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><base href='https://qa.invalid/'><style>{css}</style></head><body><div id='app'></div><script>globalThis.__RC26_ROLE__={json.dumps(role)};{AMD_LOADER}</script><script>{bundle}</script><script>require(['qa/rc26_integrated_app']);</script></body></html>"""
...
def run_role(browser,role):
    ...
    try:
        ...
        print('RC26_QA_BODY',page.locator('body').inner_text()[:4000]);print('RC26_QA_CONSOLE',console);print('RC26_QA_ERRORS',errors);raise
    ...
    for route in routes:
        ...
        navigate(page,route);page.wait_for_timeout(80);m=metrics(page);m['forbiddenLanguageHits']=[name for name,pattern in FORBIDDEN_VISIBLE_TERMS.items() if re.search(pattern,m.get('visibleLanguage') or '')];ok=m['status']=='pass' and m.get('documentLanguage')=='es-ES' and not m['forbiddenLanguageHits'] and not m['overflow'] and not m['small'] and not m['unlabeled'] and not m['unnamed'] and not m['broken'] and not m['placeholders'];route_results.append({'route':route,'ok':ok,'metrics':m})
    actions=[]
    if role=='coach':
    ...
    command_count=page.evaluate('window.__RC26_QA__.commands.length')
    privacy=page.evaluate("({role:window.__RC26_QA__.role,privateNotes:window.__RC26_QA__.app.getState().collections.privateNotes.length,reports:window.__RC26_QA__.app.getState().collections.reports.map(x=>x.id),sessions:window.__RC26_QA__.app.getState().collections.sessions.map(x=>x.id),plans:window.__RC26_QA__.app.getState().collections.trainingCycles.map(x=>x.id)})")
    final_metrics=metrics(page);final_metrics['forbiddenLanguageHits']=[name for name,pattern in FORBIDDEN_VISIBLE_TERMS.items() if re.search(pattern,final_metrics.get('visibleLanguage') or '')]
    ...
    result['ok']=all(r['ok'] for r in route_results) and all(a['ok'] for a in result['actions']) and final_metrics.get('documentLanguage')=='es-ES' and not final_metrics['forbiddenLanguageHits'] and not console and not errors and command_count>= (10 if role=='coach' else 4) and (role!='client' or (privacy['privateNotes']==0 and privacy['reports']==['report-1'] and privacy['sessions']==['session-1'] and privacy['plans']==['cycle-1']))
