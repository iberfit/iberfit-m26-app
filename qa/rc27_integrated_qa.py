...
REPORT=ROOT/'recovery'/'RC27_INTEGRATED_QA_REPORT.json'
ART=ROOT/'qa'/'rc27_integrated_artifacts'
...
def integrated_html(role: str) -> str:
    ...
    bundle=(ROOT/'qa/rc27_bundle.js').read_text(encoding='utf-8')
    return f"""<!doctype html><html lang='es-ES'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><base href='https://qa.invalid/'><style>{css}</style></head><body><div id='app'></div><script>globalThis.__RC27_ROLE__={json.dumps(role)};{AMD_LOADER}</script><script>{bundle}</script><script>require(['qa/rc27_integrated_app']);</script></body></html>"""
...
def run_role(browser,role):
    ...
    try:
        ...
        print('RC27_QA_BODY',page.locator('body').inner_text()[:4000]);print('RC27_QA_CONSOLE',console);print('RC27_QA_ERRORS',errors);raise
    ...
    command_count=page.evaluate('window.__RC27_QA__.commands.length')
    privacy=page.evaluate("(()=>{const state=window.__RC27_QA__.app.getState();const serial=JSON.stringify(state);return {role:window.__RC27_QA__.role,privateNotes:state.collections.privateNotes.length,reports:state.collections.reports.map(x=>x.id),sessions:state.collections.sessions.map(x=>x.id),plans:state.collections.trainingCycles.map(x=>x.id),containsPrivateMarker:serial.includes('NO-COMPARTIR'),containsRaw:state.collections.sessions.some(x=>Object.hasOwn(x,'raw')||Object.hasOwn(x,'body'))};})()")
    final_metrics=metrics(page);final_metrics['forbiddenLanguageHits']=[name for name,pattern in FORBIDDEN_VISIBLE_TERMS.items() if re.search(pattern,final_metrics.get('visibleLanguage') or '')];final_metrics['clientEditorialHits']=[name for name,pattern in CLIENT_FORBIDDEN_EDITORIAL_TERMS.items() if False and role=='client' and re.search(pattern,final_metrics.get('visibleLanguage') or '')]
    result={'role':role,'routes':route_results,'actions':[{'name':n,'ok':ok} for n,ok in actions],'command_count':command_count,'privacy':privacy,'console_errors':console,'page_errors':errors,'screenshot':str(shot.relative_to(ROOT)),'finalMetrics':final_metrics}
    result['ok']=all(r['ok'] for r in route_results) and all(a['ok'] for a in result['actions']) and final_metrics.get('documentLanguage')=='es-ES' and not final_metrics['forbiddenLanguageHits'] and not final_metrics['clientEditorialHits'] and not console and not errors and command_count>= (10 if role=='coach' else 4) and (role!='client' or (privacy['privateNotes']==0 and privacy['reports']==['report-1'] and privacy['sessions']==['session-1','session-2'] and privacy['plans']==['cycle-1'] and not privacy['containsPrivateMarker'] and not privacy['containsRaw']))
