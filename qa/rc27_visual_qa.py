...
ARTIFACTS = ROOT / "qa" / "rc27_visual_artifacts"
REPORT = ROOT / "recovery" / "RC27_VISUAL_QA_REPORT.json"
...
CASES = [Case(**item) for item in json.loads((ROOT / "qa" / "rc27_visual_cases.json").read_text(encoding="utf-8"))]

FORBIDDEN_VISIBLE_TERMS = {
    "CHECKIN": r"(?i)\bcheck[ -]?in(?:s)?\b",
    "WEARABLE": r"(?i)\bwearables?\b",
    "COACH": r"(?i)\bcoach(?:es)?\b",
    "ONLINE": r"(?i)\bonline\b",
    "OFFLINE": r"(?i)\boffline\b",
    "FEEDBACK": r"(?i)\bfeedback\b",
    "DASHBOARD": r"(?i)\bdashboard\b",
    "LOADING": r"(?i)\bloading\b",
    "RETRY": r"(?i)\bretry\b",
    "PENDING": r"(?i)\bpending\b",
    "REJECTED": r"(?i)\brejected\b",
    "READY": r"(?i)\bready\b",
    "CHAIR_STAND": r"(?i)\bchair stand\b",
    "STEP_TEST": r"(?i)\bstep test\b",
    "HRV": r"(?i)\bHRV\b",
    "OAUTH": r"(?i)\bOAuth\b",
    "APP": r"(?i)(?<![\w.-])app(?![\w.-])",

    "TEMPO": r"(?i)\btempo\b",
    "CLAMSHELL": r"(?i)\bclamshell\b",
    "MONSTER_WALK": r"(?i)\bmonster walk\b",
    "DEAD_BUG": r"(?i)\bdead bug\b",
    "HIP_THRUST": r"(?i)\bhip thrust\b",
    "HACK_SQUAT": r"(?i)\bhack squat\b",
    "STEP_UP_DOWN": r"(?i)\bstep-(?:up|down)\b",
    "KETTLEBELL": r"(?i)\bkettlebell\b",
    "LANDMINE": r"(?i)\blandmine\b",
    "FITBALL": r"(?i)\bfitball\b",
    "TRAP_BAR": r"(?i)\btrap bar\b",
    "MULTIPOWER": r"(?i)\bmultipower\b",
    "PRESS": r"(?i)\bpress\b",
    "CURL": r"(?i)\bcurl\b",
    "CRUNCH": r"(?i)\bcrunch\b",
    "PULLOVER": r"(?i)\bpullover\b",
    "LUNGE": r"(?i)\blunge\b",
    "SPLIT_SQUAT": r"(?i)\bsplit squat\b",
    "GOBLET": r"(?i)\bgoblet\b",
    "OVERHEAD": r"(?i)\boverhead\b",
    ...
    return page.evaluate(
        ...
        ({width}) => {
          ...
          return {
            ...
            visibleLanguage:[document.body.innerText,...all.flatMap((el)=>['aria-label','aria-description','title','placeholder','alt'].map((attr)=>el.getAttribute?.(attr)).filter(Boolean))].join('\\n'),
            qa:window.__RC27_QA__||null,
          };
        }
        """,
        {"width": case.width},
...
def validate_metrics(metrics: dict[str, Any], case: Case, console_errors: list[str], page_errors: list[str], load_ms: float) -> list[str]:
    ...
    if metrics.get("documentLanguage") != "es-ES": errors.append("DOCUMENT_LANGUAGE_NOT_ES_ES")
    visible_language = metrics.get("visibleLanguage") or ""
    language_hits = [name for name, pattern in FORBIDDEN_VISIBLE_TERMS.items() if re.search(pattern, visible_language)]
    if language_hits:
        metrics["forbiddenLanguageHits"] = language_hits
        errors.append("VISIBLE_COPY_NOT_CASTILIAN")
    else:
        metrics["forbiddenLanguageHits"] = []
    client_hits = [name for name, pattern in CLIENT_FORBIDDEN_EDITORIAL_TERMS.items() if case.role == "client" and case.route in {"planificacion","sesion","informes"} and re.search(pattern, visible_language)]
    metrics["clientEditorialHits"] = client_hits
    if client_hits: errors.append("CLIENT_EDITORIAL_LANGUAGE_EXPOSED")
    if case.role == "client" and "NO-COMPARTIR" in visible_language: errors.append("CLIENT_PRIVATE_MARKER_VISIBLE")
    return errors
...
def main() -> int:
    ...
    with sync_playwright() as pw:
        ...
        for case in CASES:
            ...
            source=(ROOT / "qa" / "rc27_visual_cases" / f"{case.name}.html").read_text(encoding="utf-8")
    ...
    contact_path=ARTIFACTS/"RC27_CONTACT_SHEET.png"; contact.save(contact_path)
    report={"version":"26.0.0-cliente-premium-final.27","generated_at":time.strftime("%Y-%m-%dT%H:%M:%SZ",time.gmtime()),"chromium":CHROMIUM,"case_count":len(results),"passed":sum(1 for r in results if r["ok"]),"failed":sum(1 for r in results if not r["ok"]),"contact_sheet":str(contact_path.relative_to(ROOT)),"results":results}
