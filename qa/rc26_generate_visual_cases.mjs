...
const outDir=path.join(__dirname,'rc26_visual_cases');
...
const cases=JSON.parse(fs.readFileSync(path.join(__dirname,'rc26_visual_cases.json'),'utf8'));
...
function stateFor(role,activeArea,scenario='normal'){
  ...
  const data={
    clients:[{id:clientId,name:'Cliente IBERFIT',modality:'Híbrido',status:'activo'},...(!isClient?[{id:otherClientId,name:'Cliente de seguimiento',modality:'En línea',status:'activo'}]:[])],
    clientProfiles:[{id:'profile-1',clientId,birthDate:'1990-01-01',objective:'Fuerza, salud y autonomía',status:'activo'}],clientAccess:[{id:'access-1',clientId,status:'activo'}],
    ...
    reports:[
      ...
      {id:'report-published',clientId,title:'Informe IRI publicado',status:'publicado',visibleToClient:true,createdAt:'2026-07-01T12:00:00Z',revision:3},
    ...
    trainingCycles:[
      ...
      {id:'plan-published',clientId,name:'Ciclo base de fuerza',title:'Ciclo base de fuerza',status:'publicado',visibleToClient:true,revision:3},
    ...
    sessions:[
      ...
      {id:'session-published',clientId,title:'Fuerza global · Sesión A',status:'publicado',visibleToClient:true,revision:3,blocks:[{id:'b1',type:'exercise',exerciseId:records[0].id,name:'Ejercicio guiado',sets:2,reps:'8',restSeconds:60,targetRpe:7,targetRir:3,alternativeId:records[1].id}]},
...
for(const item of cases){
  ...
  const html=`<!doctype html><html lang="es-ES"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><meta name="color-scheme" content="dark"><title>Auditoría visual IBERFIT</title><style>html,body{margin:0;min-width:320px;min-height:100%;background:#07150f}${css}</style></head><body data-qa-status="pass" data-qa-role="${item.role}" data-qa-route="${item.route}" data-qa-scenario="${item.scenario}">${markup}<script>window.__RC26_QA__={castellano:true,dispositivos:true,soloCosteCero:true};</script></body></html>`;
  fs.writeFileSync(path.join(outDir,`${item.name}.html`),html);
}
console.log(`Generated ${cases.length} RC26 visual cases`);
