...
const outDir=path.join(__dirname,'rc17_visual_cases');
...
const cases=JSON.parse(fs.readFileSync(path.join(__dirname,'rc17_visual_cases.json'),'utf8'));
...
function stateFor(role,activeArea,scenario='normal'){
  ...
  return createProductionState({
    ...
    canary:{active:true,scope:'allowlist',version:'26.0.0-resilience-candidate.17'},selectedClientId:clientId,activeArea,
...
console.log(`Generated ${cases.length} RC17 visual cases`);
