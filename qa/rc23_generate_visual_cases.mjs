...
const outDir=path.join(__dirname,'rc23_visual_cases');
...
const cases=JSON.parse(fs.readFileSync(path.join(__dirname,'rc23_visual_cases.json'),'utf8'));
...
function stateFor(role,activeArea,scenario='normal'){
  ...
  return createProductionState({
    ...
    identity:isClient?{id:'61227666-d8b4-4d1e-aa08-2405ad2000db',role:'client',clientId,name:'Cliente QA M26'}:{id:'2425747b-93aa-44ed-86f3-334919a1f832',role:'coach',name:'Entrenador QA M26'},
    environment:{name:'QA visual local',commandRegistry:M26_EXTENDED_COMMAND_REGISTRY},
    canary:{active:true,scope:'allowlist',version:'26.0.0-castellano-ux.23'},selectedClientId:clientId,activeArea,
    collections:{...createProductionState().collections,
      clients:isClient?[{id:clientId,name:'Cliente Prueba IBERFIT',modality:'Híbrido',status:'activo'}]:[{id:clientId,name:'Cliente Prueba IBERFIT',modality:'Híbrido',status:'activo'},{id:otherClientId,name:'Cliente en línea Seguimiento',modality:'En línea',status:'activo'}],
...
for(const item of cases){
  ...
  const html=`<!doctype html><html lang="es-ES"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><meta name="color-scheme" content="dark"><title>Auditoría visual IBERFIT</title><style>html,body{margin:0;min-width:320px;min-height:100%;background:#07150f}${css}</style></head><body data-qa-status="pass" data-qa-role="${item.role}" data-qa-route="${item.route}" data-qa-scenario="${item.scenario}">${markup}<script>window.__RC23_QA__={castellano:true,dispositivos:true,soloCosteCero:true};</script></body></html>`;
  fs.writeFileSync(path.join(outDir,`${item.name}.html`),html);
}
console.log(`Generated ${cases.length} RC23 visual cases`);
