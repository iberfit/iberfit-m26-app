...
REPORT=ROOT/'recovery'/'RC14_INTEGRATED_QA_REPORT.json'
ART=ROOT/'qa'/'rc14_integrated_artifacts'
...
def integrated_html(role: str) -> str:
    ...
    bundle=(ROOT/'qa/rc14_bundle.js').read_text(encoding='utf-8')
    return f"""<!doctype html><html lang='es'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><base href='https://qa.invalid/'><style>{css}</style></head><body><div id='app'></div><script>globalThis.__RC14_ROLE__={json.dumps(role)};{AMD_LOADER}</script><script>{bundle}</script><script>require(['qa/rc14_integrated_app']);</script></body></html>"""
...
def run_role(browser,role):
    ...
    if role=='coach':
        ...
        set_value(page,'[data-workflow-form="iri"] input[name="assessmentDate"]','2026-07-19');set_value(page,'[data-workflow-form="iri"] input[name="birthDate"]','1990-01-01');page.locator('[data-workflow-form="iri"] select[name="sexForNorms"]').select_option('female');set_value(page,'[data-workflow-form="iri"] input[name="stepFinalHr"]','150');set_value(page,'[data-workflow-form="iri"] input[name="stepOneMinuteHr"]','110');set_value(page,'[data-workflow-form="iri"] input[name="pushUps"]','12');set_value(page,'[data-workflow-form="iri"] input[name="chairStand30s"]','18');page.locator('[data-workflow-action="complete-iri"]').click();page.wait_for_timeout(300);actions.append(('iri','confirmado' in page.locator('[data-workflow-status="iri"]').inner_text().lower()))
        ...
        navigate(page,'sesion');page.wait_for_timeout(50);page.locator('[data-workflow-action="open-session-builder"]').click();page.wait_for_timeout(180);actions.append(('builder',page.locator('[data-session-action="add-exercise"]').count()>0))
    else:
        navigate(page,'actividad');page.wait_for_timeout(50)
    ...
    command_count=page.evaluate('window.__RC14_QA__.commands.length')
    ...
    result['ok']=all(r['ok'] for r in route_results) and all(a['ok'] for a in result['actions']) and not console and not errors and command_count>= (4 if role=='coach' else 2)
