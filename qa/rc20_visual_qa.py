...
ARTIFACTS = ROOT / "qa" / "rc20_visual_artifacts"
REPORT = ROOT / "recovery" / "RC20_VISUAL_QA_REPORT.json"
...
CASES = [Case(**item) for item in json.loads((ROOT / "qa" / "rc20_visual_cases.json").read_text(encoding="utf-8"))]
...
def main() -> int:
    ...
    with sync_playwright() as pw:
        ...
        for case in CASES:
            ...
            source=(ROOT / "qa" / "rc20_visual_cases" / f"{case.name}.html").read_text(encoding="utf-8")
    ...
    contact_path=ARTIFACTS/"RC20_CONTACT_SHEET.png"; contact.save(contact_path)
    report={"version":"26.0.0-performance-wearables.20","generated_at":time.strftime("%Y-%m-%dT%H:%M:%SZ",time.gmtime()),"chromium":CHROMIUM,"case_count":len(results),"passed":sum(1 for r in results if r["ok"]),"failed":sum(1 for r in results if not r["ok"]),"contact_sheet":str(contact_path.relative_to(ROOT)),"results":results}
