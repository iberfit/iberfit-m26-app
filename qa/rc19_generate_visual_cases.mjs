...
const outDir=path.join(__dirname,'rc19_visual_cases');
...
const cases=JSON.parse(fs.readFileSync(path.join(__dirname,'rc19_visual_cases.json'),'utf8'));
...
function stateFor(role,activeArea,scenario='normal'){
  ...
  return createProductionState({
    ...
    canary:{active:true,scope:'allowlist',version:'26.0.0-final-local-audit.19'},selectedClientId:clientId,activeArea,
...
console.log(`Generated ${cases.length} RC19 visual cases`);
