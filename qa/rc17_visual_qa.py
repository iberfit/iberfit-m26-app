...
ARTIFACTS = ROOT / "qa" / "rc17_visual_artifacts"
REPORT = ROOT / "recovery" / "RC17_VISUAL_QA_REPORT.json"
...
def main() -> int:
    ...
    with sync_playwright() as pw:
        ...
        for case in CASES:
            ...
            source=(ROOT / "qa" / "rc17_visual_cases" / f"{case.name}.html").read_text(encoding="utf-8")
    ...
    contact_path=ARTIFACTS/"RC17_CONTACT_SHEET.png"; contact.save(contact_path)
    report={"version":"26.0.0-resilience-candidate.17","generated_at":time.strftime("%Y-%m-%dT%H:%M:%SZ",time.gmtime()),"chromium":CHROMIUM,"case_count":len(results),"passed":sum(1 for r in results if r["ok"]),"failed":sum(1 for r in results if not r["ok"]),"contact_sheet":str(contact_path.relative_to(ROOT)),"results":results}
