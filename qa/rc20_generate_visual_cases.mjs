...
const outDir=path.join(__dirname,'rc20_visual_cases');
...
const cases=JSON.parse(fs.readFileSync(path.join(__dirname,'rc20_visual_cases.json'),'utf8'));
...
function stateFor(role,activeArea,scenario='normal'){
  ...
  return createProductionState({
    ...
    canary:{active:true,scope:'allowlist',version:'26.0.0-performance-wearables.20'},selectedClientId:clientId,activeArea,
    collections:{...createProductionState().collections,
      ...
      wearableConnections:[{id:'wear-1',clientId,provider:'apple_health',status:'conectado',lastSyncedAt:'2026-07-19T12:00:00Z',scopes:['steps','sleepMinutes','restingHeartRate']}],
      wearableDailySummaries:[
        {id:'wear-day-1',clientId,provider:'apple_health',date:'2026-07-17',steps:7200,activeMinutes:38,sleepMinutes:425,restingHeartRate:60,hrvMs:44,activeEnergyKcal:480,workoutMinutes:30,quality:'alta',sourceUpdatedAt:'2026-07-17T22:00:00Z'},
        {id:'wear-day-2',clientId,provider:'apple_health',date:'2026-07-18',steps:8600,activeMinutes:52,sleepMinutes:455,restingHeartRate:58,hrvMs:49,activeEnergyKcal:590,workoutMinutes:45,quality:'alta',sourceUpdatedAt:'2026-07-18T22:00:00Z'},
        {id:'wear-day-3',clientId,provider:'apple_health',date:'2026-07-19',steps:7900,activeMinutes:44,sleepMinutes:440,restingHeartRate:59,hrvMs:47,activeEnergyKcal:530,workoutMinutes:35,quality:'alta',sourceUpdatedAt:'2026-07-19T15:00:00Z'}
...
for(const item of cases){
  ...
  const html=`<!doctype html><html lang="es-CL"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><meta name="color-scheme" content="dark"><title>${item.name}</title><style>html,body{margin:0;min-width:320px;min-height:100%;background:#07150f}${css}</style></head><body data-qa-status="pass" data-qa-role="${item.role}" data-qa-route="${item.route}" data-qa-scenario="${item.scenario}">${markup}<script>window.__RC20_QA__={wearables:true};</script></body></html>`;
  fs.writeFileSync(path.join(outDir,`${item.name}.html`),html);
}
console.log(`Generated ${cases.length} RC20 visual cases`);
