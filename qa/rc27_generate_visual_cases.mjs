...
const outDir=path.join(__dirname,'rc27_visual_cases');
...
const cases=JSON.parse(fs.readFileSync(path.join(__dirname,'rc27_visual_cases.json'),'utf8'));
...
for(const item of cases){
  ...
  const html=`<!doctype html><html lang="es-ES"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><meta name="color-scheme" content="dark"><title>Auditoría visual IBERFIT</title><style>html,body{margin:0;min-width:320px;min-height:100%;background:#07150f}${css}</style></head><body data-qa-status="pass" data-qa-role="${item.role}" data-qa-route="${item.route}" data-qa-scenario="${item.scenario}">${markup}<script>window.__RC27_QA__={castellano:true,dispositivos:true,soloCosteCero:true};</script></body></html>`;
  fs.writeFileSync(path.join(outDir,`${item.name}.html`),html);
}
console.log(`Generated ${cases.length} RC27 visual cases`);
