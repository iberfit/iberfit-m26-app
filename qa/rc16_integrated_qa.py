...
REPORT=ROOT/'recovery'/'RC16_INTEGRATED_QA_REPORT.json'
ART=ROOT/'qa'/'rc16_integrated_artifacts'
...
def integrated_html(role: str) -> str:
    ...
    bundle=(ROOT/'qa/rc16_bundle.js').read_text(encoding='utf-8')
    return f"""<!doctype html><html lang='es'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><base href='https://qa.invalid/'><style>{css}</style></head><body><div id='app'></div><script>globalThis.__RC16_ROLE__={json.dumps(role)};{AMD_LOADER}</script><script>{bundle}</script><script>require(['qa/rc16_integrated_app']);</script></body></html>"""
...
def run_role(browser,role):
    ...
    try:
        ...
        print('RC16_QA_BODY',page.locator('body').inner_text()[:4000]);print('RC16_QA_CONSOLE',console);print('RC16_QA_ERRORS',errors);raise
    ...
    command_count=page.evaluate('window.__RC16_QA__.commands.length')
