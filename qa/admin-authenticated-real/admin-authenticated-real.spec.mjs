...
test('real QA Coach Admin identity chooses Admin after WebAuthn and remains usable across desktop tablet and mobile',async({browser})=>{
  ...
  try{
    ...
    await expect(roleChoice,'Coach Admin identity must choose which authorized IBERFIT app to enter after WebAuthn').toBeVisible({timeout:30_000});
    const provisionalCoachShell=page.locator('.m26-shell[data-m26-role="coach"]');
    await expect(provisionalCoachShell,'Primary Coach app remains inert until an app is chosen').toHaveAttribute('inert','');
    await expect(provisionalCoachShell).toHaveAttribute('aria-hidden','true');
    const chooseCoach=roleChoice.locator('[data-m26-switch-role="coach"]');
    const chooseAdmin=roleChoice.locator('[data-m26-switch-role="admin"]');
    await expect(chooseCoach).toBeVisible();
    await expect(chooseAdmin).toBeVisible();
    await expect(roleChoice.locator('[data-m26-switch-role="client"]'),'Client app must remain isolated and must not be offered').toHaveCount(0);
    evidence.appChoice={shown:true,authorized:['coach','admin'],selected:'admin'};
