...
const dist=path.join(root,'dist','m26-publicacion-premium-candidate');
fs.rmSync(dist,{recursive:true,force:true});fs.mkdirSync(dist,{recursive:true});
function copy(source,target){const from=path.join(root,source),to=path.join(dist,target);if(!fs.existsSync(from))throw new Error(`RC26_BUILD_SOURCE_MISSING:${source}`);fs.mkdirSync(path.dirname(to),{recursive:true});fs.cpSync(from,to,{recursive:true});}
