...
check('Informe conserva CSS CSP, ajuste A4 por página y bloqueo ante desborde',/mode:'direct-window'/.test(reports)&&/openWindow\('about:blank','_blank'\)/.test(reports)&&/document\.write/.test(reports)&&/data-iri-report-stylesheet/.test(reports)&&/fitReportPages/.test(reports)&&/reportPageContentFits/.test(reports)&&/M26_IRI_REPORT_LAYOUT_NOT_READY/.test(reports)&&/Encabezados y pies de página/.test(reports)&&/data-iri-report-shell/.test(reportShell)&&/iri-report\.css/.test(reportShell)&&/document\.body\.replaceChildren/.test(reportPage)&&!/document\.head\.append/.test(reportPage)&&/\.report-page-content/.test(reportCss)&&/iri-report-fit-82/.test(reportCss));
