...
check('Informe conserva CSS permitido por CSP, valida maquetación y mantiene ruta de respaldo',/mode:'direct-window'/.test(reports)&&/openWindow\('about:blank','_blank'\)/.test(reports)&&/document\.write/.test(reports)&&/data-iri-report-stylesheet/.test(reports)&&/reportLayoutReady/.test(reports)&&/M26_IRI_REPORT_LAYOUT_NOT_READY/.test(reports)&&/data-iri-report-shell/.test(reportShell)&&/iri-report\.css/.test(reportShell)&&/document\.body\.replaceChildren/.test(reportPage)&&!/document\.head\.append/.test(reportPage)&&/\.pdf-page/.test(reportCss));
