...
const VERSION = '26.0.0-canary.33';
const RELEASE = 'IBERFIT_M26_CANARY_RC33';
const DEFAULT_BRANCH = 'canary/rc33';
const SERVICE_WORKER_VERSION = 'm26-rc33-canary-v1';
const PREVIOUS_SERVICE_WORKER_VERSION = 'm26-rc32-canary-v1';
...
const validationOnly =
  ...
  'true';
if (!key) throw new Error('RC33_RUNTIME_PUBLISHABLE_KEY_MISSING');
if (/service[_-]?role/iu.test(key)) {
  throw new Error('RC33_RUNTIME_SERVICE_ROLE_FORBIDDEN');
}
...
if (!fs.existsSync(path.dirname(runtimeTarget))) {
  throw new Error(`RC33_RUNTIME_BUILD_MISSING:${buildDir}`);
}
...
if (!fs.existsSync(serviceWorkerTarget)) {
  throw new Error('RC33_RUNTIME_SERVICE_WORKER_MISSING');
}
...
if (canaryServiceWorker === originalServiceWorker) {
  throw new Error('RC33_RUNTIME_SERVICE_WORKER_VERSION_NOT_FOUND');
}
if (
  ...
  throw new Error('RC33_RUNTIME_SERVICE_WORKER_VERSION_INCOMPLETE');
}
...
if (duplicates.length) {
  throw new Error(
    `RC33_RUNTIME_DUPLICATE_ASSETS:${[...new Set(duplicates)].join(',')}`,
  );
}
