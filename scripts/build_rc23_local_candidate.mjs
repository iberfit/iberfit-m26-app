...
const dist=path.join(root,'dist','m26-castellano-ux-candidate');
fs.rmSync(dist,{recursive:true,force:true});fs.mkdirSync(dist,{recursive:true});
function copy(source,target){const from=path.join(root,source),to=path.join(dist,target);if(!fs.existsSync(from))throw new Error(`RC23_BUILD_SOURCE_MISSING:${source}`);fs.mkdirSync(path.dirname(to),{recursive:true});fs.cpSync(from,to,{recursive:true});}
...
const meta={version:'26.0.0-castellano-ux.23',status:'not_deployed',deployable:false,localValidationOnly:true,productionModified:false,productionDeployed:false,builtAt:new Date().toISOString(),files:files.length,totalBytes,budgets:{javascriptBytes,cssBytes,jsonBytes,javascriptLimit:800000,cssLimit:150000,totalLimit:3600000},budgetOk:javascriptBytes<=800000&&cssBytes<=150000&&totalBytes<=3600000,locale:'es-ES'};
