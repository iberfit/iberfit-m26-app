...
function readRequired(file, label) {
  if (!fs.existsSync(file)) throw new Error(`RC41_PATCH_MISSING:${label}`);
  return fs.readFileSync(file, 'utf8').replace(/\r\n?/gu, '\n');
}

function replaceOnce(source, needle, replacement, label) {
  const first = source.indexOf(needle);
  if (first < 0) throw new Error(`RC41_PATCH_ANCHOR_MISSING:${label}`);
  if (source.indexOf(needle, first + needle.length) >= 0) {
    throw new Error(`RC41_PATCH_ANCHOR_DUPLICATE:${label}`);
  }
...
if (transport.includes(PRODUCTION_REF)) {
  throw new Error('RC41_PATCH_PRODUCTION_REF_REMAINS');
}
if (!transport.includes(CANARY_REF)) {
  throw new Error('RC41_PATCH_CANARY_REF_MISSING');
}
if (iriExternalReport.includes(PRODUCTION_REF)) {
  throw new Error('RC41_PATCH_IRI_PRODUCTION_REF_REMAINS');
}
if (!iriExternalReport.includes(CANARY_REF)) {
  throw new Error('RC41_PATCH_IRI_CANARY_REF_MISSING');
}
if (!transport.includes("QA_AUTHORIZED_EMAILS=new Set(['iberfit.cl@gmail.com'])")) {
  throw new Error('RC41_PATCH_CARLOS_ALLOWLIST_MISSING');
}
if (!application.includes("locationLike?.origin")) {
  throw new Error('RC41_PATCH_RECOVERY_ORIGIN_MISSING');
}
...
function walk(directory) {
  ...
  for (const entry of entries) {
    ...
    if (updated === source || updated.includes(PRODUCTION_ORIGIN)) {
      throw new Error(`RC41_CSP_PATCH_FAILED:${absolute}`);
    }
...
if (cspPatched.length === 0) {
  throw new Error('RC41_CSP_SOURCE_NOT_FOUND');
}

console.log(JSON.stringify({
  ok: true,
  release: 'IBERFIT_M26_CANARY_RC41_PWA_DIAGNOSTICS',
