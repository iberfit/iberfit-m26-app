...
const checks=[
 ...
 ['visual-castilian',visual.case_count===22&&visual.passed===22&&visual.failed===0&&visualNoHits],
 ...
 ['service-worker-cache-bumped',/m26-rc23/.test(read('public/m26/sw.js'))&&/m26-rc22/.test(read('public/m26/sw.js'))],
];
const results=checks.map(([name,value])=>({name,ok:Boolean(value)}));const failed=results.filter((x)=>!x.ok);const report={release:'IBERFIT_M26_CASTELLANO_UX_RC23',version:VERSION,generatedAt:new Date().toISOString(),passed:results.length-failed.length,total:results.length,failed:failed.length,checks:results,ok:failed.length===0,note:'Audita únicamente texto visible y atributos accesibles; los identificadores internos y nombres propios de proveedores se conservan.'};
fs.writeFileSync('recovery/RC23_CASTELLANO_GATE_REPORT.json',JSON.stringify(report,null,2)+'\n');for(const item of results)console.log(`${item.ok?'PASS':'FAIL'} ${item.name}`);console.log(`\n${report.passed}/${report.total} PASS`);if(failed.length)process.exit(1);
