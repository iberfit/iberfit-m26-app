...
import {isQuadrupedalLocomotion,movementSupportContract,supportObservationInstruction,supportPairObservationInstruction,quadrupedSupportObservationPass,quadrupedPairSupportObservationPass} from './auto-factory-movement-contract.mjs';
...
async function qaRequest({proxy,token,content,label}){
  const body={messages:[{role:'system',content:'Output exactly one JSON object. Be adversarial and fail ambiguity.'},{role:'user',content}],temperature:0,stream:false,max_completion_tokens:2600,chat_template_kwargs:{enable_thinking:false,preserve_thinking:false},response_format:{type:'json_object'}};
  const response=await fetchWithTransientRetry(proxy.replace(/\/generate$/,'/qa'),{method:'POST',headers:{authorization:`Bearer ${token}`,'content-type':'application/json'},body:JSON.stringify(body),redirect:'error'},{label});if(!response.ok)throw new Error(`${label}_HTTP_${response.status}:${(await response.text()).slice(0,900)}`);
...
async function validateRawStart({claim,plan,startFile,outDir,proxy,token,reviewAttempt=0}){
  const exercise=claim.claim.exercise;const inferred=Boolean(plan.anatomy_inferred);const minConfidence=inferred?0.985:0.97;const quadruped=isQuadrupedalLocomotion(exercise);const supportContract=movementSupportContract(exercise);const supportInstruction=supportObservationInstruction(exercise);
  const keys=['start_matches_plan','support_topology','critical_body_visible','no_portrait_or_rest_pose'];
  const rubric=[
    'You are the fail-closed START phase reviewer for IBERFIT Exercise Media System v1. Judge the generated photograph before it can become the continuity reference for FINAL.',
    ...
    supportContract,
    'Reject if the image is a related movement rather than the exact START, uses the wrong weight-bearing contacts/supports, becomes a portrait/rest pose, hides critical joints or requires guessing about the planned body geometry.',
    supportInstruction,
    'Confidence calibration is mandatory and evidence-based. Use 0.99-1.00 only when every required relationship is clearly visible and unambiguous; 0.97-0.98 for a valid START with only minor non-critical uncertainty; <=0.96 when phase, supports, contacts or joint relationships require guessing. Do not default to 0.95.',
    `Return ONLY JSON: {${keys.map(k=>`"${k}":boolean`).join(',')},"confidence":number,"issues":[string]${quadruped?',"support_observation":object':''}}.`
  ].filter(Boolean).join('\n');
  const review=await qaRequest({proxy,token,label:'RAW_START_QA',content:[{type:'image_url',image_url:{url:dataUri(startFile)}},{type:'text',text:rubric}]});
  const checks={};for(const key of keys)checks[key]=review[key]===true;if(quadruped)checks.support_topology=checks.support_topology&&quadrupedSupportObservationPass(exercise,review.support_observation);const confidence=Number(review.confidence);const pass=keys.every(k=>checks[k]===true)&&Number.isFinite(confidence)&&confidence>=minConfidence;const output={schema:'iberfit.exercise.media.auto.raw-start-qa.v1',exercise_id:exercise.id,pass,confidence:Number.isFinite(confidence)?confidence:0,min_confidence:minConfidence,checks,support_observation:quadruped?(review.support_observation||null):null,issues:Array.isArray(review.issues)?review.issues.map(String).slice(0,12):[],anatomy_inferred:inferred,review_attempt:reviewAttempt,reviewed_at:new Date().toISOString()};const serialized=`${JSON.stringify(output,null,2)}\n`;fs.writeFileSync(path.join(outDir,'raw-start-qa.json'),serialized);fs.writeFileSync(path.join(outDir,`raw-start-qa-attempt-${reviewAttempt}.json`),serialized);return output;
}
async function validateRawPair({claim,plan,startFile,finalFile,outDir,proxy,token,reviewAttempt=0}){
  const exercise=claim.claim.exercise;const inferred=Boolean(plan.anatomy_inferred);const minConfidence=inferred?0.985:0.97;const quadruped=isQuadrupedalLocomotion(exercise);const supportContract=movementSupportContract(exercise);const supportInstruction=supportPairObservationInstruction(exercise);
  const keys=['start_matches_plan','final_matches_plan','same_identity','same_scene_and_camera','equipment_continuity','grip_support_continuity','support_topology','critical_body_visible','no_portrait_or_rest_pose'];
  const rubric=[
    ...
    supportInstruction,
    'Confidence calibration is mandatory and evidence-based. Use 0.99-1.00 only when every required relationship is clearly visible and unambiguous; 0.97-0.98 for valid phases with only minor non-critical visual uncertainty; <=0.96 when any phase, grip, equipment connection, support topology, identity, joint relationship or scene continuity requires guessing. Do not default to 0.95.',
    `Return ONLY JSON: {${keys.map(k=>`"${k}":boolean`).join(',')},"confidence":number,"issues":[string]${quadruped?',"support_observation":object':''}}.`
  ].filter(Boolean).join('\n');
  const review=await qaRequest({proxy,token,label:'RAW_PHASE_QA',content:[{type:'image_url',image_url:{url:dataUri(startFile)}},{type:'image_url',image_url:{url:dataUri(finalFile)}},{type:'text',text:rubric}]});
  const checks={};for(const key of keys)checks[key]=review[key]===true;if(quadruped)checks.support_topology=checks.support_topology&&quadrupedPairSupportObservationPass(exercise,review.support_observation);const confidence=Number(review.confidence);const pass=keys.every(k=>checks[k]===true)&&Number.isFinite(confidence)&&confidence>=minConfidence;const output={schema:'iberfit.exercise.media.auto.raw-phase-qa.v1',exercise_id:exercise.id,pass,confidence:Number.isFinite(confidence)?confidence:0,min_confidence:minConfidence,checks,support_observation:quadruped?(review.support_observation||null):null,issues:Array.isArray(review.issues)?review.issues.map(String).slice(0,12):[],anatomy_inferred:inferred,review_attempt:reviewAttempt,reviewed_at:new Date().toISOString()};const serialized=`${JSON.stringify(output,null,2)}\n`;fs.writeFileSync(path.join(outDir,'raw-phase-qa.json'),serialized);fs.writeFileSync(path.join(outDir,`raw-phase-qa-attempt-${reviewAttempt}.json`),serialized);return output;
}
async function main(){
  ...
  const supportContract=movementSupportContract(exercise);
  const generateOnce=async(repairAttempt=0,repairIssues=[])=>{
    const repairInstruction=repairAttempt>0?`REPAIR PASS ${repairAttempt}: strict raw QA rejected the previous ${phase.toUpperCase()}. Correct every listed defect while preserving identity, premium scene language and valid equipment geometry. Previous QA issues: ${repairIssues.join(' | ')}. This is a biomechanics repair, not a stylistic variation. Make the required ${phase.toUpperCase()} support/contact geometry unmistakable.`:'';
    const prompt=[
      ...
      `Camera language is strictly ${plan.camera}. Do not substitute a different viewpoint. Keep the entire athlete, relevant hands, feet, joints, support points and all equipment visible. Leave useful background breathing room, especially toward the upper corners, without compromising biomechanics.`,
      ...
      'This is an exercise demonstration, never a portrait or rest pose. Any grip, cable, handle, machine contact or bodyweight support required by the phase must remain visibly connected and physically plausible. Never fold or cross the arms as a casual pose unless the canonical exercise explicitly requires that action.',
  ...
  if(phase==='start'){
    ...
    for(let attempt=0;attempt<=START_REPAIR_ATTEMPTS;attempt+=1){
      ...
      generated=await generateOnce(attempt,repairIssues);review=await validateRawStart({claim,plan,startFile:generated.file,outDir,proxy,token,reviewAttempt:attempt});if(review.pass)return;repairIssues=review.issues;
    }
    const failed=review?Object.entries(review.checks).filter(([,ok])=>ok!==true).map(([key])=>key).join(',')||'confidence':'unknown';throw new Error(`RAW_START_QA_FAILED:${review?.confidence??0}:${failed}`);
  }
  let generated=null;let review=null;let repairIssues=[];
