...
async function qaRequest({proxy,token,content,label,maxCompletionTokens}){
  const body={messages:[{role:'system',content:'Output exactly one JSON object. Be adversarial and fail ambiguity.'},{role:'user',content}],temperature:0,stream:false,max_completion_tokens:maxCompletionTokens,chat_template_kwargs:{enable_thinking:false,preserve_thinking:false},response_format:{type:'json_object'}};
...
async function validateRawPair({claim,plan,startFile,finalFile,outDir,proxy,token,reviewAttempt=0}){
  ...
  const review=await qaRequest({proxy,token,label:'RAW_PHASE_QA',maxCompletionTokens:2600,content:[{type:'image_url',image_url:{url:dataUri(startFile)}},{type:'image_url',image_url:{url:dataUri(finalFile)}},{type:'text',text:rubric}]});
  const checks={};for(const key of keys)checks[key]=review[key]===true;if(hardSupport)checks.support_topology=checks.support_topology&&supportPairObservationPass(exercise,review.support_observation);const confidence=Number(review.confidence);const pass=keys.every(k=>checks[k]===true)&&Number.isFinite(confidence)&&confidence>=minConfidence;const output={schema:'iberfit.exercise.media.auto.raw-phase-qa.v1',exercise_id:exercise.id,pass,confidence:Number.isFinite(confidence)?confidence:0,min_confidence:minConfidence,checks,support_observation:hardSupport?(review.support_observation||null):null,issues:Array.isArray(review.issues)?review.issues.map(String).slice(0,12):[],anatomy_inferred:inferred,review_attempt:reviewAttempt,reviewed_at:new Date().toISOString()};const serialized=`${JSON.stringify(output,null,2)}\n`;fs.writeFileSync(path.join(outDir,'raw-phase-qa.json'),serialized);fs.writeFileSync(path.join(outDir,`raw-phase-qa-attempt-${reviewAttempt}.json`),serialized);return output;
}
async function main(){
