...
async function main(){
  ...
  const text=[
    ...
    'Confidence calibration is mandatory and evidence-based. Use 0.99-1.00 when left-chest fabric is clearly visible and the anchor is safe without guessing; 0.97-0.98 when the fabric is visible with only minor perspective or folds but placement remains clearly safe; <=0.96 when occlusion, distortion, uncertain shirt boundary or geometry requires guessing. Do not default to 0.90 or 0.95.',
  ...
  const body={messages:[{role:'system',content:'Output one complete JSON object only. Be conservative and precise.'},{role:'user',content:[{type:'image_url',image_url:{url:dataUri}},{type:'text',text}]}],temperature:0,stream:false,max_completion_tokens:900,chat_template_kwargs:{enable_thinking:false,preserve_thinking:false},response_format:{type:'json_object'}};
