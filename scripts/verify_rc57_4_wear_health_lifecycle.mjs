...
pass(
  ...
  provider.includes("ExerciseUpdateCallback") &&
    provider.includes("setUpdateCallback(exerciseCallback)") &&
    provider.includes("clearUpdateCallbackAsync(exerciseCallback)")
);

pass(
  "rc57-4-capabilities-gate",
  provider.includes("getCapabilitiesAsync()") &&
    provider.includes("ExerciseType.WORKOUT") &&
    provider.includes("supportedDataTypes") &&
    provider.includes("DataType.HEART_RATE_BPM")
);

pass(
  "rc57-4-exercise-lifecycle",
  provider.includes("startExerciseAsync(config)") &&
    provider.includes("pauseExerciseAsync()") &&
    provider.includes("resumeExerciseAsync()") &&
    provider.includes("endExerciseAsync()")
);
...
pass(
  ...
  provider.includes(
    ...
    provider.includes("IBERFITHeartRateSample(")
);
...
pass(
  ...
  provider.includes(
    ...
    wear.includes("dataLayer.sendSample(sample.toDataLayerJson())")
);
