...
const validation=read('recovery/RC20_LOCAL_VALIDATION.json');
const quality=read('recovery/m26-rc20-quality-gate-results.json');
const visual=read('recovery/RC20_VISUAL_QA_REPORT.json');
const integrated=read('recovery/RC20_INTEGRATED_QA_REPORT.json');
const graph=read('recovery/RC20_MODULE_GRAPH_REPORT.json');
const protectedLayers=read('recovery/RC20_PROTECTED_BASELINE_COMPARISON.json');
const build=read('dist/m26-performance-wearables-candidate/version.json');
const dependencies=read('recovery/RC20_DEPENDENCY_AUDIT.json');
const remote=read('recovery/RC20_REMOTE_VALIDATION_STATUS.json');
const qaVerification=read('recovery/RC20_QA_REPORT_VERIFICATION.json');
const webRoot=path.join(root,'dist','m26-performance-wearables-candidate');
const webEntries=walk(webRoot,webRoot);
write('recovery/RC20_WEB_SHA256_MANIFEST.json',{release:'IBERFIT_M26_WEB_CANARY_RC20',version:build.version,generatedAt:new Date().toISOString(),count:webEntries.length,totalBytes:webEntries.reduce((s,x)=>s+x.size,0),entries:webEntries});

const summary={
  release:'IBERFIT_M26_PERFORMANCE_WEARABLES_RC20',
  ...
  historicalEvidence:validation.inheritedEvidence,
  visualQa:{passed:visual.passed,total:visual.case_count,failed:visual.failed},
  integratedQa:{passed:integrated.passed,total:integrated.total,routes:integrated.results.reduce((s,x)=>s+x.routes.length,0),actions:integrated.results.reduce((s,x)=>s+x.actions.length,0)},
...
write('recovery/RC20_SUMMARY.json',summary);
const excluded=new Set(['recovery/RC20_SHA256_MANIFEST.json']);
...
write('recovery/RC20_SHA256_MANIFEST.json',{release:summary.release,version:summary.version,generatedAt:new Date().toISOString(),excluded:[...excluded],count:entries.length,totalBytes:entries.reduce((s,x)=>s+x.size,0),entries});
