...
function protectedComparison(){
  ...
  fs.writeFileSync(abs('recovery/RC23_PROTECTED_BASELINE_COMPARISON.json'),JSON.stringify(report,null,2)+'\n');
  return report;
}
...
const build=json('dist/m26-castellano-ux-candidate/version.json');
const graph=json('recovery/RC23_MODULE_GRAPH_REPORT.json');
const visual=json('recovery/RC23_VISUAL_QA_REPORT.json');
const integrated=json('recovery/RC23_INTEGRATED_QA_REPORT.json');
const castellano=json('recovery/RC23_CASTELLANO_GATE_REPORT.json');
const qaVerification=json('recovery/RC23_QA_REPORT_VERIFICATION.json');
...
const checks=[
 ...
 ['module-graph',graph.ok&&graph.version===VERSION&&graph.modules>=59&&graph.missing.length===0],
 ...
 ['visual-qa',visual.version===VERSION&&visual.case_count===22&&visual.passed===22&&visual.failed===0&&visualNoLanguageHits],
 ...
 ['service-worker-version',/m26-rc23/.test(sw)&&/m26-rc22/.test(sw)&&/NEVER_CACHE_PREFIXES/.test(sw)],
 ...
 ['rc23-docs',exists('docs/RC23_CASTELLANO_UX.md')&&/castellano/i.test(read('docs/RC23_CASTELLANO_UX.md'))],
 ['rc23-tests',exists('tests/m26_rc23_castellano_ui.test.mjs')&&/castellano/i.test(read('tests/m26_rc23_castellano_ui.test.mjs'))],
 ...
 ['runtime-cache-coherent',!/26\.0\.0-state-performance-ux\.22/.test(runtime)&&/VERSION='m26-rc23'/.test(sw)&&/PREVIOUS_VERSION='m26-rc22'/.test(sw)],
];
...
fs.writeFileSync(abs('recovery/m26-rc23-quality-gate-results.json'),JSON.stringify(report,null,2)+'\n');
