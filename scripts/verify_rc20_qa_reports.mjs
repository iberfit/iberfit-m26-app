...
const VERSION='26.0.0-performance-wearables.20';
const MAX_AGE_MS=24*60*60*1000;
const definitions=[
  {name:'visual',path:'recovery/RC20_VISUAL_QA_REPORT.json',expected:{passed:18,totalKey:'case_count',total:18,failed:0}},
  {name:'integrated',path:'recovery/RC20_INTEGRATED_QA_REPORT.json',expected:{passed:2,totalKey:'total',total:2}},
];
const checks=[];
...
const report={
  ...
  checks,
  note:'Este verificador no ejecuta Chromium. Confirma que las evidencias visual e integrada fueron generadas por RC20, están completas y son recientes. Las QA se ejecutan como pasos independientes para evitar procesos Chromium huérfanos en el orquestador.'
};
writeFileSync('recovery/RC20_QA_REPORT_VERIFICATION.json',JSON.stringify(report,null,2)+'\n');
