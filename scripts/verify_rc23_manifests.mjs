...
function verify(manifestPath,base){
  ...
  for(const entry of manifest.entries){
    ...
    const size=fs.statSync(file).size,digest=sha(file);
...
const full=verify('recovery/RC23_SHA256_MANIFEST.json','');
const web=verify('recovery/RC23_WEB_SHA256_MANIFEST.json','dist/m26-castellano-ux-candidate');
const report={release:'IBERFIT_M26_CASTELLANO_UX_RC23',version:'26.0.0-castellano-ux.23',generatedAt:new Date().toISOString(),full,web,ok:full.ok&&web.ok};
fs.writeFileSync(path.join(root,'recovery','RC23_MANIFEST_VERIFICATION.json'),JSON.stringify(report,null,2)+'\n');
