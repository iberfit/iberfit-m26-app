...
function protectedComparison(){
  ...
  fs.writeFileSync(abs('recovery/RC26_PROTECTED_BASELINE_COMPARISON.json'),JSON.stringify(report,null,2)+'\n');
  return report;
}
...
const build=json('dist/m26-publicacion-premium-candidate/version.json');
const graph=json('recovery/RC26_MODULE_GRAPH_REPORT.json');
const visual=json('recovery/RC26_VISUAL_QA_REPORT.json');
const integrated=json('recovery/RC26_INTEGRATED_QA_REPORT.json');
const castellano=json('recovery/RC26_CASTELLANO_GATE_REPORT.json');
const qaVerification=json('recovery/RC26_QA_REPORT_VERIFICATION.json');
...
const visualNoLanguageHits=visual.results.every((item)=>item.ok&&item.metrics?.documentLanguage==='es-ES'&&item.metrics?.forbiddenLanguageHits?.length===0&&!item.console_errors.length&&!item.page_errors.length);
const integratedNoLanguageHits=integrated.results.every((item)=>item.ok&&item.routes.every((entry)=>entry.metrics?.documentLanguage==='es-ES'&&entry.metrics?.forbiddenLanguageHits?.length===0)&&item.finalMetrics?.documentLanguage==='es-ES'&&item.finalMetrics?.forbiddenLanguageHits?.length===0&&!item.console_errors.length&&!item.page_errors.length);
...
const checks=[
 ...
 ['module-graph',graph.ok&&graph.version===VERSION&&graph.modules>=63&&graph.missing.length===0],
 ...
 ['service-worker-version',/m26-rc26/.test(sw)&&/m26-rc23/.test(sw)&&/NEVER_CACHE_PREFIXES/.test(sw)],
 ...
 ['rc26-docs',exists('docs/RC26_PUBLICACION_PREMIUM_Y_VISIBILIDAD.md')&&/aprobar/i.test(read('docs/RC26_PUBLICACION_PREMIUM_Y_VISIBILIDAD.md'))&&/publicar/i.test(read('docs/RC26_PUBLICACION_PREMIUM_Y_VISIBILIDAD.md'))],
 ['rc26-tests',exists('tests/m26_rc26_publication_premium.test.mjs')&&/publicar exige aprobación previa/i.test(read('tests/m26_rc26_publication_premium.test.mjs'))],
 ...
 ['runtime-cache-coherent',new RegExp(VERSION.replaceAll('.','\\.')).test(runtime)&&/VERSION='m26-rc26'/.test(sw)&&/PREVIOUS_VERSION='m26-rc23'/.test(sw)],
];
...
fs.writeFileSync(abs('recovery/m26-rc26-quality-gate-results.json'),JSON.stringify(report,null,2)+'\n');
