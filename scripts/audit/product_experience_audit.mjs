...
export function evaluateProductExperience(files={}){
  ...
  if(has(css,/backdrop-filter\s*:/u)&&has(css,/linear-gradient\s*\(/u))strengths.push(strength('PREMIUM_VISUAL_LAYERING','aesthetic','La superficie usa profundidad, transparencia y gradientes de forma sistemática.','Se detectan backdrop-filter y gradientes.'));
  ...
  return Object.freeze({
    version:VERSION,
    result:'PASS',
    strengths:Object.freeze(uniq(strengths)),
    opportunities:Object.freeze(ordered),
    summary:Object.freeze({
      strengths:uniq(strengths).length,
  ...
  async function visit(current){
    ...
    for(const entry of entries){
      ...
      if(entry.isDirectory()){
        ...
        await visit(full);
      }else if(/\.(?:js|mjs|css|html|webmanifest)$/u.test(entry.name)){
...
function markdown(report,generatedAt){
  ...
  lines.push('','> Las oportunidades son no bloqueantes: orientan evolución estética, app-like y de entrenamiento. Los fallos de seguridad/contrato siguen perteneciendo a los gates estrictos.','');
  return `${lines.join('\n')}\n`;
}
