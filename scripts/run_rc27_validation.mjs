...
const tasks=[
  ...
  {name:'build',command:process.execPath,args:['scripts/build_rc27_local_candidate.mjs'],timeout:60000},
  {name:'module-graph',command:process.execPath,args:['scripts/verify_rc27_module_graph.mjs'],timeout:60000},
  {name:'qa-report-verification',command:process.execPath,args:['scripts/verify_rc27_qa_reports.mjs'],timeout:60000},
  {name:'castellano-gate',command:process.execPath,args:['scripts/m26_rc27_castellano_gate.mjs'],timeout:60000},
  {name:'quality-gate',command:process.execPath,args:['scripts/m26_rc27_client_premium_gate.mjs'],timeout:120000},
];
...
const quality=safeJson('recovery/m26-rc27-quality-gate-results.json'),visual=safeJson('recovery/RC27_VISUAL_QA_REPORT.json'),integrated=safeJson('recovery/RC27_INTEGRATED_QA_REPORT.json'),castellano=safeJson('recovery/RC27_CASTELLANO_GATE_REPORT.json');
...
writeFileSync('recovery/RC27_LOCAL_VALIDATION.json',JSON.stringify(report,null,2)+'\n');writeFileSync('recovery/RC27_VALIDATION_OUTPUT.txt',results.map((item)=>`===== ${item.name} =====\n${item.stdout}\n${item.stderr}\nSTATUS ${item.status} TIMEOUT ${item.timedOut} DURATION_MS ${item.durationMs}`).join('\n\n')+'\n');
writeFileSync('recovery/RC27_REMOTE_VALIDATION_STATUS.json',JSON.stringify({release:RELEASE,version:VERSION,localValidationPassed:report.ok,remoteGatesPassed:false,productionModified:false,productionDeployed:false,deployable:false,publicationLifecycleValidatedLocally:quality?.ok===true,roleProjectionValidatedLocally:integrated?.results?.find((x)=>x.role==='client')?.privacy||null,pendingExternalGates:report.pendingExternalGates},null,2)+'\n');
console.log(`RC27 tests: ${tests.passed}/${tests.total}`);console.log(`RC27 quality: ${quality?.passed||0}/${quality?.total||0}`);console.log(`RC27 castellano: ${castellano?.passed||0}/${castellano?.total||0}`);console.log(`RC27 visual: ${visual?.passed||0}/${visual?.case_count||0}`);console.log(`RC27 integrated: ${integrated?.passed||0}/${integrated?.total||0}`);console.log(report.ok?'RC27 LOCAL VALIDATION PASS':'RC27 LOCAL VALIDATION FAIL');if(!report.ok)process.exit(1);
