...
function readRequired(file, label) {
  if (!fs.existsSync(file)) throw new Error(`RC40_PATCH_MISSING:${label}`);
  return fs.readFileSync(file, 'utf8').replace(/\r\n?/gu, '\n');
}

function replaceOnce(source, needle, replacement, label) {
  const first = source.indexOf(needle);
  if (first < 0) throw new Error(`RC40_PATCH_ANCHOR_MISSING:${label}`);
  if (source.indexOf(needle, first + needle.length) >= 0) {
    throw new Error(`RC40_PATCH_ANCHOR_DUPLICATE:${label}`);
  }
...
transport = replaceOnce(
  ...
  [
    ...
    "  const canary = host === 'm26-canary.iberfit.cl' || configuredCanary;",
    "  const qaOnly = canary || Boolean(raw?.qaOnly);",
    "  const enabled = Boolean(raw?.enabled) && (local || (qaOnly ? canary : exactRemote));",
  ].join('\n'),
...
if (transport.includes(PRODUCTION_REF)) {
  throw new Error('RC40_PATCH_PRODUCTION_REF_REMAINS');
}
...
if (!application.includes("locationLike?.origin")) {
  throw new Error('RC40_PATCH_RECOVERY_ORIGIN_MISSING');
}
